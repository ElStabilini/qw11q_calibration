
# Single QUA script generated at 2025-02-16 11:38:05.411941
# QUA library version: 1.2.1

from qm import CompilerOptionArguments
from qm.qua import *

with program() as prog:
    v1 = declare(int, )
    v2 = declare(fixed, )
    v3 = declare(fixed, )
    v4 = declare(int, )
    v5 = declare(fixed, )
    v6 = declare(fixed, )
    v7 = declare(int, )
    v8 = declare(fixed, )
    v9 = declare(fixed, )
    v10 = declare(int, )
    v11 = declare(fixed, )
    v12 = declare(fixed, )
    v13 = declare(int, )
    set_dc_offset("B1/flux", "single", -0.25498290735703955)
    set_dc_offset("D1/flux", "single", -0.4530870218982913)
    set_dc_offset("B2/flux", "single", 0.10729465913983083)
    set_dc_offset("D2/flux", "single", -0.2224873138863394)
    set_dc_offset("B3/flux", "single", 0.2226188560782865)
    set_dc_offset("D3/flux", "single", 0.05128942239382605)
    set_dc_offset("B4/flux", "single", 0.114743772754262)
    set_dc_offset("D4/flux", "single", -0.08416990010352905)
    set_dc_offset("B5/flux", "single", 0.0)
    set_dc_offset("D5/flux", "single", 0.05418914676504196)
    wait((4+(0*((Cast.to_int(v2)+Cast.to_int(v3))+Cast.to_int(v4)))), "B1/acquisition")
    wait((4+(0*((Cast.to_int(v5)+Cast.to_int(v6))+Cast.to_int(v7)))), "B2/acquisition")
    wait((4+(0*((Cast.to_int(v8)+Cast.to_int(v9))+Cast.to_int(v10)))), "B3/acquisition")
    wait((4+(0*((Cast.to_int(v11)+Cast.to_int(v12))+Cast.to_int(v13)))), "B4/acquisition")
    with for_(v1,0,(v1<2048),(v1+1)):
        align()
        play("-1885320965307138871", "B1/drive")
        wait(11, "B1/flux")
        play("-6912180221144202672", "B1/flux")
        wait(46, "B1/drive")
        play("3274127046900791126", "B1/drive")
        wait(66, "B1/acquisition")
        measure("-7269982027912257413", "B1/acquisition", None, dual_demod.full("cos", "sin", v2), dual_demod.full("minus_sin", "cos", v3))
        assign(v4, Cast.to_int((((v2*0.9967019904271117)-(v3*0.08114888957116839))>0.0009208204328015715)))
        r1 = declare_stream()
        save(v4, r1)
        play("9181858318004187397", "B2/drive")
        wait(11, "B2/flux")
        play("-6912180221144202672", "B2/flux")
        wait(46, "B2/drive")
        play("-8503795126481802350", "B2/drive")
        wait(66, "B2/acquisition")
        measure("-9087730669898779636", "B2/acquisition", None, dual_demod.full("cos", "sin", v5), dual_demod.full("minus_sin", "cos", v6))
        assign(v7, Cast.to_int((((v5*0.7386661925213004)-(v6*0.6740714027653786))>0.0026726729978320107)))
        r2 = declare_stream()
        save(v7, r2)
        play("-3579424934842844975", "B3/drive")
        wait(11, "B3/flux")
        play("-6912180221144202672", "B3/flux")
        wait(46, "B3/drive")
        play("-2818334305619283106", "B3/drive")
        wait(66, "B3/acquisition")
        measure("-4041517210051566932", "B3/acquisition", None, dual_demod.full("cos", "sin", v8), dual_demod.full("minus_sin", "cos", v9))
        assign(v10, Cast.to_int((((v8*-0.8459998192018453)-(v9*-0.5331831823214654))>0.0026818753539089865)))
        r3 = declare_stream()
        save(v10, r3)
        play("-5671910444091142186", "B4/drive")
        wait(11, "B4/flux")
        play("-6912180221144202672", "B4/flux")
        wait(46, "B4/drive")
        play("-4910819814867580317", "B4/drive")
        wait(66, "B4/acquisition")
        measure("2690639089030047106", "B4/acquisition", None, dual_demod.full("cos", "sin", v11), dual_demod.full("minus_sin", "cos", v12))
        assign(v13, Cast.to_int((((v11*0.4781750072295442)-(v12*0.8782645742946856))>-0.00048643881283392637)))
        r4 = declare_stream()
        save(v13, r4)
        wait(25501, "B1/drive")
        wait(25536, "B4/flux")
        wait(25536, "B2/flux")
        wait(25536, "B1/flux")
        wait(25001, "B4/acquisition")
        wait(25536, "B3/flux")
        wait(25001, "B3/acquisition")
        wait(25501, "B3/drive")
        wait(25001, "B1/acquisition")
        wait(25501, "B4/drive")
        wait(25501, "B2/drive")
        wait(25001, "B2/acquisition")
        play("-1885320965307138871", "B1/drive")
        wait(11, "B1/flux")
        play("-2389862451054199356", "B1/flux")
        wait(46, "B1/drive")
        play("3274127046900791126", "B1/drive")
        wait(66, "B1/acquisition")
        measure("-7269982027912257413", "B1/acquisition", None, dual_demod.full("cos", "sin", v2), dual_demod.full("minus_sin", "cos", v3))
        assign(v4, Cast.to_int((((v2*0.9967019904271117)-(v3*0.08114888957116839))>0.0009208204328015715)))
        save(v4, r1)
        play("9181858318004187397", "B2/drive")
        wait(11, "B2/flux")
        play("-2389862451054199356", "B2/flux")
        wait(46, "B2/drive")
        play("-8503795126481802350", "B2/drive")
        wait(66, "B2/acquisition")
        measure("-9087730669898779636", "B2/acquisition", None, dual_demod.full("cos", "sin", v5), dual_demod.full("minus_sin", "cos", v6))
        assign(v7, Cast.to_int((((v5*0.7386661925213004)-(v6*0.6740714027653786))>0.0026726729978320107)))
        save(v7, r2)
        play("-3579424934842844975", "B3/drive")
        wait(11, "B3/flux")
        play("-2389862451054199356", "B3/flux")
        wait(46, "B3/drive")
        play("-2818334305619283106", "B3/drive")
        wait(66, "B3/acquisition")
        measure("-4041517210051566932", "B3/acquisition", None, dual_demod.full("cos", "sin", v8), dual_demod.full("minus_sin", "cos", v9))
        assign(v10, Cast.to_int((((v8*-0.8459998192018453)-(v9*-0.5331831823214654))>0.0026818753539089865)))
        save(v10, r3)
        play("-5671910444091142186", "B4/drive")
        wait(11, "B4/flux")
        play("-2389862451054199356", "B4/flux")
        wait(46, "B4/drive")
        play("-4910819814867580317", "B4/drive")
        wait(66, "B4/acquisition")
        measure("2690639089030047106", "B4/acquisition", None, dual_demod.full("cos", "sin", v11), dual_demod.full("minus_sin", "cos", v12))
        assign(v13, Cast.to_int((((v11*0.4781750072295442)-(v12*0.8782645742946856))>-0.00048643881283392637)))
        save(v13, r4)
        wait(25501, "B1/drive")
        wait(25536, "B4/flux")
        wait(25536, "B2/flux")
        wait(25536, "B1/flux")
        wait(25001, "B4/acquisition")
        wait(25536, "B3/flux")
        wait(25001, "B3/acquisition")
        wait(25501, "B3/drive")
        wait(25001, "B1/acquisition")
        wait(25501, "B4/drive")
        wait(25501, "B2/drive")
        wait(25001, "B2/acquisition")
        wait(25000, )
    with stream_processing():
        r1.buffer(2).average().save("-7269982027912257413_B1|acquisition_shots")
        r2.buffer(2).average().save("-9087730669898779636_B2|acquisition_shots")
        r3.buffer(2).average().save("-4041517210051566932_B3|acquisition_shots")
        r4.buffer(2).average().save("2690639089030047106_B4|acquisition_shots")


config = {
    "version": 1,
    "controllers": {
        "con4": {
            "type": "opx1",
            "analog_outputs": {
                "1": {
                    "offset": -0.25498290735703955,
                    "filter": {},
                },
                "2": {
                    "offset": 0.10729465913983083,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                },
                "3": {
                    "offset": 0.2226188560782865,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                },
                "4": {
                    "offset": 0.114743772754262,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                },
                "5": {
                    "offset": 0.0,
                    "filter": {},
                },
            },
            "digital_outputs": {},
            "analog_inputs": {
                "1": {
                    "offset": 0,
                },
                "2": {
                    "offset": 0,
                },
            },
        },
        "con9": {
            "type": "opx1",
            "analog_outputs": {
                "3": {
                    "offset": -0.4530870218982913,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                },
                "4": {
                    "offset": -0.2224873138863394,
                    "filter": {
                        "feedforward": [1.0891790415038731, -1.024484298837039],
                        "feedback": [0.935305257333166],
                    },
                },
                "5": {
                    "offset": 0.05128942239382605,
                    "filter": {
                        "feedforward": [1.1298143371682787, -0.9007185757251136],
                        "feedback": [0.7709042385568349],
                    },
                },
                "6": {
                    "offset": -0.08416990010352905,
                    "filter": {},
                },
                "7": {
                    "offset": 0.05418914676504196,
                    "filter": {},
                },
            },
            "digital_outputs": {},
            "analog_inputs": {
                "1": {
                    "offset": 0,
                },
                "2": {
                    "offset": 0,
                },
            },
        },
        "con2": {
            "type": "opx1",
            "analog_outputs": {
                "3": {
                    "offset": 0.0,
                    "filter": {},
                },
                "4": {
                    "offset": 0.0,
                    "filter": {},
                },
                "1": {
                    "offset": 0.0,
                    "filter": {},
                },
                "2": {
                    "offset": 0.0,
                    "filter": {},
                },
                "7": {
                    "offset": 0.0,
                    "filter": {},
                },
                "8": {
                    "offset": 0.0,
                    "filter": {},
                },
            },
            "digital_outputs": {
                "3": {},
                "1": {},
                "7": {},
            },
            "analog_inputs": {
                "1": {
                    "offset": 0.0,
                    "gain_db": 10,
                },
                "2": {
                    "offset": 0.0,
                    "gain_db": 10,
                },
            },
        },
        "con3": {
            "type": "opx1",
            "analog_outputs": {
                "1": {
                    "offset": 0.0,
                    "filter": {},
                },
                "2": {
                    "offset": 0.0,
                    "filter": {},
                },
                "7": {
                    "offset": 0.0,
                    "filter": {},
                },
                "8": {
                    "offset": 0.0,
                    "filter": {},
                },
            },
            "digital_outputs": {
                "1": {},
                "7": {},
            },
            "analog_inputs": {
                "1": {
                    "offset": 0,
                },
                "2": {
                    "offset": 0,
                },
            },
        },
    },
    "octaves": {
        "octave2": {
            "connectivity": "con2",
            "RF_outputs": {
                "2": {
                    "LO_frequency": 4900000000.0,
                    "gain": 0.0,
                    "LO_source": "internal",
                    "output_mode": "triggered",
                },
                "1": {
                    "LO_frequency": 7370000000.0,
                    "gain": -10.0,
                    "LO_source": "internal",
                    "output_mode": "triggered",
                },
                "4": {
                    "LO_frequency": 5900000000.0,
                    "gain": 0.0,
                    "LO_source": "internal",
                    "output_mode": "triggered",
                },
            },
            "RF_inputs": {
                "1": {
                    "LO_frequency": 7370000000.0,
                    "LO_source": "internal",
                    "IF_mode_I": "direct",
                    "IF_mode_Q": "direct",
                },
            },
        },
        "octave3": {
            "connectivity": "con3",
            "RF_outputs": {
                "1": {
                    "LO_frequency": 5800000000.0,
                    "gain": 0.0,
                    "LO_source": "internal",
                    "output_mode": "triggered",
                },
                "4": {
                    "LO_frequency": 6700000000.0,
                    "gain": 0.0,
                    "LO_source": "internal",
                    "output_mode": "triggered",
                },
            },
            "RF_inputs": {},
        },
    },
    "elements": {
        "B1/flux": {
            "singleInput": {
                "port": ('con4', 1),
            },
            "intermediate_frequency": 0,
            "operations": {
                "-6912180221144202672": "-6912180221144202672",
                "-2389862451054199356": "-2389862451054199356",
            },
        },
        "D1/flux": {
            "singleInput": {
                "port": ('con9', 3),
            },
            "intermediate_frequency": 0,
            "operations": {},
        },
        "B2/flux": {
            "singleInput": {
                "port": ('con4', 2),
            },
            "intermediate_frequency": 0,
            "operations": {
                "-6912180221144202672": "-6912180221144202672",
                "-2389862451054199356": "-2389862451054199356",
            },
        },
        "D2/flux": {
            "singleInput": {
                "port": ('con9', 4),
            },
            "intermediate_frequency": 0,
            "operations": {},
        },
        "B3/flux": {
            "singleInput": {
                "port": ('con4', 3),
            },
            "intermediate_frequency": 0,
            "operations": {
                "-6912180221144202672": "-6912180221144202672",
                "-2389862451054199356": "-2389862451054199356",
            },
        },
        "D3/flux": {
            "singleInput": {
                "port": ('con9', 5),
            },
            "intermediate_frequency": 0,
            "operations": {},
        },
        "B4/flux": {
            "singleInput": {
                "port": ('con4', 4),
            },
            "intermediate_frequency": 0,
            "operations": {
                "-6912180221144202672": "-6912180221144202672",
                "-2389862451054199356": "-2389862451054199356",
            },
        },
        "D4/flux": {
            "singleInput": {
                "port": ('con9', 6),
            },
            "intermediate_frequency": 0,
            "operations": {},
        },
        "B5/flux": {
            "singleInput": {
                "port": ('con4', 5),
            },
            "intermediate_frequency": 0,
            "operations": {},
        },
        "D5/flux": {
            "singleInput": {
                "port": ('con9', 7),
            },
            "intermediate_frequency": 0,
            "operations": {},
        },
        "B1/drive": {
            "RF_inputs": {
                "port": ('octave2', 2),
            },
            "digitalInputs": {
                "output_switch": {
                    "port": ('con2', 3),
                    "delay": 57,
                    "buffer": 18,
                },
            },
            "intermediate_frequency": 100388701.0,
            "operations": {
                "-1885320965307138871": "-1885320965307138871",
                "3274127046900791126": "3274127046900791126",
            },
        },
        "B4/acquisition": {
            "RF_inputs": {
                "port": ('octave2', 1),
            },
            "RF_outputs": {
                "port": ('octave2', 1),
            },
            "digitalInputs": {
                "output_switch": {
                    "port": ('con2', 1),
                    "delay": 57,
                    "buffer": 18,
                },
            },
            "intermediate_frequency": 330300527.0,
            "time_of_flight": 224.0,
            "smearing": 0.0,
            "operations": {
                "2690639089030047106": "2690639089030047106_B4/acquisition",
            },
        },
        "B3/acquisition": {
            "RF_inputs": {
                "port": ('octave2', 1),
            },
            "RF_outputs": {
                "port": ('octave2', 1),
            },
            "digitalInputs": {
                "output_switch": {
                    "port": ('con2', 1),
                    "delay": 57,
                    "buffer": 18,
                },
            },
            "intermediate_frequency": 110622376.0,
            "time_of_flight": 224.0,
            "smearing": 0.0,
            "operations": {
                "-4041517210051566932": "-4041517210051566932_B3/acquisition",
            },
        },
        "B3/drive": {
            "RF_inputs": {
                "port": ('octave3', 1),
            },
            "digitalInputs": {
                "output_switch": {
                    "port": ('con3', 1),
                    "delay": 57,
                    "buffer": 18,
                },
            },
            "intermediate_frequency": -115376210.0,
            "operations": {
                "-3579424934842844975": "-3579424934842844975",
                "-2818334305619283106": "-2818334305619283106",
            },
        },
        "B1/acquisition": {
            "RF_inputs": {
                "port": ('octave2', 1),
            },
            "RF_outputs": {
                "port": ('octave2', 1),
            },
            "digitalInputs": {
                "output_switch": {
                    "port": ('con2', 1),
                    "delay": 57,
                    "buffer": 18,
                },
            },
            "intermediate_frequency": -237451236.0,
            "time_of_flight": 224.0,
            "smearing": 0.0,
            "operations": {
                "-7269982027912257413": "-7269982027912257413_B1/acquisition",
            },
        },
        "B4/drive": {
            "RF_inputs": {
                "port": ('octave3', 4),
            },
            "digitalInputs": {
                "output_switch": {
                    "port": ('con3', 7),
                    "delay": 57,
                    "buffer": 18,
                },
            },
            "intermediate_frequency": 109615374.0,
            "operations": {
                "-5671910444091142186": "-5671910444091142186",
                "-4910819814867580317": "-4910819814867580317",
            },
        },
        "B2/drive": {
            "RF_inputs": {
                "port": ('octave2', 4),
            },
            "digitalInputs": {
                "output_switch": {
                    "port": ('con2', 7),
                    "delay": 57,
                    "buffer": 18,
                },
            },
            "intermediate_frequency": 63761228.0,
            "operations": {
                "9181858318004187397": "9181858318004187397",
                "-8503795126481802350": "-8503795126481802350",
            },
        },
        "B2/acquisition": {
            "RF_inputs": {
                "port": ('octave2', 1),
            },
            "RF_outputs": {
                "port": ('octave2', 1),
            },
            "digitalInputs": {
                "output_switch": {
                    "port": ('con2', 1),
                    "delay": 57,
                    "buffer": 18,
                },
            },
            "intermediate_frequency": 10040944.0,
            "time_of_flight": 224.0,
            "smearing": 0.0,
            "operations": {
                "-9087730669898779636": "-9087730669898779636_B2/acquisition",
            },
        },
    },
    "pulses": {
        "-1885320965307138871": {
            "length": 40,
            "waveforms": {
                "I": "-1885320965307138871_i",
                "Q": "-1885320965307138871_q",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-5492222400611121274": {
            "length": 16,
            "waveforms": {
                "single": "-5492222400611121274",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-7269982027912257413_B1/acquisition": {
            "length": 2000.0,
            "waveforms": {
                "I": "-3597628208875341953_i",
                "Q": "-3597628208875341953_q",
            },
            "digital_marker": "ON",
            "operation": "measurement",
            "integration_weights": {
                "cos": "cosine_weights_B1/acquisition",
                "sin": "sine_weights_B1/acquisition",
                "minus_sin": "minus_sine_weights_B1/acquisition",
            },
        },
        "9181858318004187397": {
            "length": 40,
            "waveforms": {
                "I": "9181858318004187397_i",
                "Q": "9181858318004187397_q",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-9087730669898779636_B2/acquisition": {
            "length": 2000.0,
            "waveforms": {
                "I": "191989489022899436_i",
                "Q": "191989489022899436_q",
            },
            "digital_marker": "ON",
            "operation": "measurement",
            "integration_weights": {
                "cos": "cosine_weights_B2/acquisition",
                "sin": "sine_weights_B2/acquisition",
                "minus_sin": "minus_sine_weights_B2/acquisition",
            },
        },
        "-3579424934842844975": {
            "length": 40,
            "waveforms": {
                "I": "-3579424934842844975_i",
                "Q": "-3579424934842844975_q",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-4041517210051566932_B3/acquisition": {
            "length": 2000.0,
            "waveforms": {
                "I": "-676466526619785515_i",
                "Q": "-676466526619785515_q",
            },
            "digital_marker": "ON",
            "operation": "measurement",
            "integration_weights": {
                "cos": "cosine_weights_B3/acquisition",
                "sin": "sine_weights_B3/acquisition",
                "minus_sin": "minus_sine_weights_B3/acquisition",
            },
        },
        "-5671910444091142186": {
            "length": 40,
            "waveforms": {
                "I": "-5671910444091142186_i",
                "Q": "-5671910444091142186_q",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "2690639089030047106_B4/acquisition": {
            "length": 2000.0,
            "waveforms": {
                "I": "2330723448246942021_i",
                "Q": "2330723448246942021_q",
            },
            "digital_marker": "ON",
            "operation": "measurement",
            "integration_weights": {
                "cos": "cosine_weights_B4/acquisition",
                "sin": "sine_weights_B4/acquisition",
                "minus_sin": "minus_sine_weights_B4/acquisition",
            },
        },
        "-5270728341412406026": {
            "length": 16,
            "waveforms": {
                "single": "-5270728341412406026",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-6418550158589274799": {
            "length": 16,
            "waveforms": {
                "single": "-6418550158589274799",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-37518524385641680": {
            "length": 16,
            "waveforms": {
                "single": "-37518524385641680",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-8619147853570476064": {
            "length": 16,
            "waveforms": {
                "single": "-8619147853570476064",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "2160241163617525183": {
            "length": 16,
            "waveforms": {
                "single": "2160241163617525183",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-192124505144769037": {
            "length": 16,
            "waveforms": {
                "single": "-192124505144769037",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "1233913405639371658": {
            "length": 16,
            "waveforms": {
                "single": "1233913405639371658",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "2005635182858397826": {
            "length": 16,
            "waveforms": {
                "single": "2005635182858397826",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "3431673093642538521": {
            "length": 16,
            "waveforms": {
                "single": "3431673093642538521",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "7238844999885162172": {
            "length": 16,
            "waveforms": {
                "single": "7238844999885162172",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "7460339059083877420": {
            "length": 16,
            "waveforms": {
                "single": "7460339059083877420",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "7183144655667875206": {
            "length": 16,
            "waveforms": {
                "single": "7183144655667875206",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-8788645326622507333": {
            "length": 16,
            "waveforms": {
                "single": "-8788645326622507333",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "1464247641275318685": {
            "length": 16,
            "waveforms": {
                "single": "1464247641275318685",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-3555435509595742987": {
            "length": 16,
            "waveforms": {
                "single": "-3555435509595742987",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "966882260494260329": {
            "length": 20,
            "waveforms": {
                "single": "966882260494260329",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-8403705110227432949": {
            "length": 20,
            "waveforms": {
                "single": "-8403705110227432949",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-1136181762393860876": {
            "length": 20,
            "waveforms": {
                "single": "-1136181762393860876",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "9116711205503965142": {
            "length": 20,
            "waveforms": {
                "single": "9116711205503965142",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "1449356148982414773": {
            "length": 24,
            "waveforms": {
                "single": "1449356148982414773",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-7132273180202419611": {
            "length": 24,
            "waveforms": {
                "single": "-7132273180202419611",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-751241545998786492": {
            "length": 24,
            "waveforms": {
                "single": "-751241545998786492",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-1899063363175655265": {
            "length": 24,
            "waveforms": {
                "single": "-1899063363175655265",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-1677569303976940017": {
            "length": 28,
            "waveforms": {
                "single": "-1677569303976940017",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-4002127386063776470": {
            "length": 28,
            "waveforms": {
                "single": "-4002127386063776470",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "520190384026226846": {
            "length": 28,
            "waveforms": {
                "single": "520190384026226846",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-7147164672495323523": {
            "length": 28,
            "waveforms": {
                "single": "-7147164672495323523",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-1292629087581865633": {
            "length": 32,
            "waveforms": {
                "single": "-1292629087581865633",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "4271661946051563531": {
            "length": 32,
            "waveforms": {
                "single": "4271661946051563531",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-5969552838430996306": {
            "length": 32,
            "waveforms": {
                "single": "-5969552838430996306",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "1126624659620016478": {
            "length": 32,
            "waveforms": {
                "single": "1126624659620016478",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-2068560836227686534": {
            "length": 36,
            "waveforms": {
                "single": "-2068560836227686534",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-2345755239643688748": {
            "length": 36,
            "waveforms": {
                "single": "-2345755239643688748",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-2124261180444973500": {
            "length": 36,
            "waveforms": {
                "single": "-2124261180444973500",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "8557594164649947687": {
            "length": 36,
            "waveforms": {
                "single": "8557594164649947687",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "6205228495887653467": {
            "length": 40,
            "waveforms": {
                "single": "6205228495887653467",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "3880670413800817014": {
            "length": 40,
            "waveforms": {
                "single": "3880670413800817014",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-1739320964049899116": {
            "length": 40,
            "waveforms": {
                "single": "-1739320964049899116",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "735633127369269961": {
            "length": 40,
            "waveforms": {
                "single": "735633127369269961",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "2161671038153410656": {
            "length": 44,
            "waveforms": {
                "single": "2161671038153410656",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-4589052013593251692": {
            "length": 44,
            "waveforms": {
                "single": "-4589052013593251692",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-6198464231832708480": {
            "length": 44,
            "waveforms": {
                "single": "-6198464231832708480",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-2391292325590084829": {
            "length": 44,
            "waveforms": {
                "single": "-2391292325590084829",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "8388096691597916418": {
            "length": 48,
            "waveforms": {
                "single": "8388096691597916418",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "7240274874421047645": {
            "length": 48,
            "waveforms": {
                "single": "7240274874421047645",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "5758536619419619984": {
            "length": 48,
            "waveforms": {
                "single": "5758536619419619984",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "5039677179439846380": {
            "length": 48,
            "waveforms": {
                "single": "5039677179439846380",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "5261171238638561628": {
            "length": 52,
            "waveforms": {
                "single": "5261171238638561628",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "7625215090816122029": {
            "length": 52,
            "waveforms": {
                "single": "7625215090816122029",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-5257237949260000423": {
            "length": 52,
            "waveforms": {
                "single": "-5257237949260000423",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-5035743890061285175": {
            "length": 52,
            "waveforms": {
                "single": "-5035743890061285175",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-8402275235691547476": {
            "length": 56,
            "waveforms": {
                "single": "-8402275235691547476",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-8180781176492832228": {
            "length": 56,
            "waveforms": {
                "single": "-8180781176492832228",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "3016551382855339578": {
            "length": 56,
            "waveforms": {
                "single": "3016551382855339578",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "2395225614968646034": {
            "length": 56,
            "waveforms": {
                "single": "2395225614968646034",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "2616719674167361282": {
            "length": 60,
            "waveforms": {
                "single": "2616719674167361282",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-749811671462901019": {
            "length": 60,
            "waveforms": {
                "single": "-749811671462901019",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "4814479362170528145": {
            "length": 60,
            "waveforms": {
                "single": "4814479362170528145",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-3379371743641197453": {
            "length": 60,
            "waveforms": {
                "single": "-3379371743641197453",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "3001659890562435666": {
            "length": 64,
            "waveforms": {
                "single": "3001659890562435666",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-3876737124422255809": {
            "length": 64,
            "waveforms": {
                "single": "-3876737124422255809",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "6902651892765745438": {
            "length": 64,
            "waveforms": {
                "single": "6902651892765745438",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "2847053909803308309": {
            "length": 64,
            "waveforms": {
                "single": "2847053909803308309",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "4273091820587449004": {
            "length": 68,
            "waveforms": {
                "single": "4273091820587449004",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "6748045912006618081": {
            "length": 68,
            "waveforms": {
                "single": "6748045912006618081",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "1128054534155901951": {
            "length": 68,
            "waveforms": {
                "single": "1128054534155901951",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-8168720658876312098": {
            "length": 68,
            "waveforms": {
                "single": "-8168720658876312098",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-1072543160825299314": {
            "length": 72,
            "waveforms": {
                "single": "-1072543160825299314",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-4267728656673002326": {
            "length": 72,
            "waveforms": {
                "single": "-4267728656673002326",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-4046234597474287078": {
            "length": 72,
            "waveforms": {
                "single": "-4046234597474287078",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-4323429000890289292": {
            "length": 72,
            "waveforms": {
                "single": "-4323429000890289292",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-516257094647665641": {
            "length": 76,
            "waveforms": {
                "single": "-516257094647665641",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "4006060675442337675": {
            "length": 76,
            "waveforms": {
                "single": "4006060675442337675",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-3661294381079212694": {
            "length": 76,
            "waveforms": {
                "single": "-3661294381079212694",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "3283707827749389633": {
            "length": 76,
            "waveforms": {
                "single": "3283707827749389633",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-7133674280342917920": {
            "length": 80,
            "waveforms": {
                "single": "-7133674280342917920",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-6912180221144202672": {
            "length": 80,
            "waveforms": {
                "single": "-6912180221144202672",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-2389862451054199356": {
            "length": 80,
            "waveforms": {
                "single": "-2389862451054199356",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "3274127046900791126": {
            "length": 40,
            "waveforms": {
                "I": "3274127046900791126_i",
                "Q": "3274127046900791126_q",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-8503795126481802350": {
            "length": 40,
            "waveforms": {
                "I": "-8503795126481802350_i",
                "Q": "-8503795126481802350_q",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-2818334305619283106": {
            "length": 40,
            "waveforms": {
                "I": "-2818334305619283106_i",
                "Q": "-2818334305619283106_q",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
        "-4910819814867580317": {
            "length": 40,
            "waveforms": {
                "I": "-4910819814867580317_i",
                "Q": "-4910819814867580317_q",
            },
            "digital_marker": "ON",
            "operation": "control",
        },
    },
    "waveforms": {
        "-1885320965307138871_i": {
            "samples": [-0.00022142740157317373, -0.0002826829761606054, -0.00035425117413348615, -0.0004356288820668947, -0.0005254580382221801, -0.0006213869286926811, -0.0007199955255671921, -0.0008168119984833208, -0.0009064422734077392, -0.0009828245001067717, -0.0010396060379057538, -0.0010706235080575682, -0.0010704488576227164, -0.0010349490619260794, -0.0009617969948137154, -0.0008508685989463798, -0.0007044682624354848, -0.0005273402858847709, -0.00032644791808218227, -0.00011052957492162269, 0.00011052957492162535, 0.0003264479180821849, 0.0005273402858847735, 0.0007044682624354872, 0.0008508685989463821, 0.0009617969948137175, 0.001034949061926081, 0.0010704488576227182, 0.00107062350805757, 0.0010396060379057551, 0.000982824500106773, 0.0009064422734077401, 0.0008168119984833217, 0.0007199955255671927, 0.0006213869286926815, 0.0005254580382221805, 0.000435628882066895, 0.00035425117413348636, 0.0002826829761606056, 0.0002214274015731739],
            "type": "arbitrary",
        },
        "-1885320965307138871_q": {
            "samples": [0.0011180555187915678, 0.0015045081475491685, 0.0019931494632565378, 0.0025995569652476707, 0.0033378972155056613, 0.004219497446825634, 0.005251249417242201, 0.006433965280360931, 0.007760843544895696, 0.009216229744590979, 0.010774864198537374, 0.012401792672522064, 0.01405307218212388, 0.015677334902548905, 0.0172181839630987, 0.018617295840194316, 0.019818008261921365, 0.020769094336384877, 0.021428376161292048] + [0.02176582398456596] * 2 + [0.021428376161292048, 0.020769094336384877, 0.019818008261921365, 0.018617295840194316, 0.0172181839630987, 0.015677334902548905, 0.01405307218212388, 0.012401792672522064, 0.010774864198537374, 0.009216229744590979, 0.007760843544895696, 0.006433965280360931, 0.005251249417242201, 0.004219497446825634, 0.0033378972155056613, 0.0025995569652476707, 0.0019931494632565378, 0.0015045081475491685, 0.0011180555187915678],
            "type": "arbitrary",
        },
        "-5492222400611121274": {
            "samples": [0.25] + [0.0] * 15,
            "type": "arbitrary",
        },
        "-3597628208875341953_i": {
            "sample": 0.0031,
            "type": "constant",
        },
        "-3597628208875341953_q": {
            "sample": 0.0,
            "type": "constant",
        },
        "9181858318004187397_i": {
            "samples": [0.0001847802794911338, 0.0002358978110714119, 0.00029562118555058855, 0.00036353055679122937, 0.0004384926277133374, 0.0005185449024836567, 0.000600833381512208, 0.0006816263402774137, 0.0007564224456091563, 0.0008201631077734924, 0.000867547073578488, 0.0008934310281524939, 0.0008932852830643261, 0.0008636608457810521, 0.0008026157388505544, 0.0007100464369202522, 0.0005878759426368721, 0.0004400633558467776, 0.0002724194797661072, 9.223648744893448e-05, -9.22364874489315e-05, -0.00027241947976610427, -0.00044006335584677477, -0.0005878759426368692, -0.0007100464369202496, -0.000802615738850552, -0.0008636608457810499, -0.0008932852830643241, -0.0008934310281524921, -0.0008675470735784865, -0.0008201631077734911, -0.0007564224456091552, -0.0006816263402774129, -0.0006008333815122073, -0.0005185449024836561, -0.00043849262771333695, -0.00036353055679122905, -0.0002956211855505882, -0.0002358978110714117, -0.00018478027949113364],
            "type": "arbitrary",
        },
        "9181858318004187397_q": {
            "samples": [0.0012493039327485741, 0.0016811221929452756, 0.0022271250587747485, 0.0029047186704985986, 0.0037297326012476254, 0.004714823786424647, 0.00586769300677935, 0.007189247754539428, 0.008671888112107319, 0.010298121937161474, 0.01203972431763831, 0.013857637745628482, 0.01570276077461938, 0.017517695516533722, 0.019239424678246013, 0.020802778144181227, 0.022144442064578468, 0.023207176028356858, 0.023943850873928398] + [0.024320911659934084] * 2 + [0.023943850873928398, 0.023207176028356858, 0.022144442064578468, 0.020802778144181227, 0.019239424678246013, 0.017517695516533722, 0.01570276077461938, 0.013857637745628482, 0.01203972431763831, 0.010298121937161474, 0.008671888112107319, 0.007189247754539428, 0.00586769300677935, 0.004714823786424647, 0.0037297326012476254, 0.0029047186704985986, 0.0022271250587747485, 0.0016811221929452756, 0.0012493039327485741],
            "type": "arbitrary",
        },
        "191989489022899436_i": {
            "sample": 0.0021,
            "type": "constant",
        },
        "191989489022899436_q": {
            "sample": 0.0,
            "type": "constant",
        },
        "-3579424934842844975_i": {
            "samples": [0.00030171729028092756, 0.0003851842227735331, 0.0004827031504638384, 0.0005935885302881006, 0.0007159898654021671, 0.0008467027071136329, 0.000981066920557875, 0.0011129891833639896, 0.0012351195226318389, 0.0013391980526670656, 0.0014165686563095033, 0.001458833104968909, 0.0014585951260395835, 0.0014102230542588634, 0.0013105459442409456, 0.00115939475528007, 0.0009599094498731598, 0.0007185546187270059, 0.00044481839447977256, 0.00015060775497668046, -0.00015060775497667656, -0.00044481839447976866, -0.0007185546187270022, -0.0009599094498731563, -0.0011593947552800666, -0.0013105459442409426, -0.0014102230542588608, -0.001458595126039581, -0.001458833104968907, -0.0014165686563095015, -0.0013391980526670638, -0.0012351195226318376, -0.0011129891833639883, -0.0009810669205578741, -0.0008467027071136323, -0.0007159898654021665, -0.0005935885302881001, -0.0004827031504638381, -0.00038518422277353286, -0.00030171729028092735],
            "type": "arbitrary",
        },
        "-3579424934842844975_q": {
            "samples": [0.0016409510779196645, 0.0022081410314293824, 0.002925311583561483, 0.00381532556527005, 0.004898974998745269, 0.006192884670996909, 0.007707169498132072, 0.009443021464190989, 0.011390458136038771, 0.01352650371967906, 0.015814085010857203, 0.018201900274243128, 0.02062545513863524, 0.023009357920839617, 0.025270835892482323, 0.02732428860950993, 0.029086553818695706, 0.030482446681665623, 0.031450063408251405] + [0.03194532984183734] * 2 + [0.031450063408251405, 0.030482446681665623, 0.029086553818695706, 0.02732428860950993, 0.025270835892482323, 0.023009357920839617, 0.02062545513863524, 0.018201900274243128, 0.015814085010857203, 0.01352650371967906, 0.011390458136038771, 0.009443021464190989, 0.007707169498132072, 0.006192884670996909, 0.004898974998745269, 0.00381532556527005, 0.002925311583561483, 0.0022081410314293824, 0.0016409510779196645],
            "type": "arbitrary",
        },
        "-676466526619785515_i": {
            "sample": 0.0017,
            "type": "constant",
        },
        "-676466526619785515_q": {
            "sample": 0.0,
            "type": "constant",
        },
        "-5671910444091142186_i": {
            "samples": [0.0003245581697577897, 0.00041434379264958276, 0.0005192451877881844, 0.0006385249144990762, 0.000770192387926079, 0.0009108005732581571, 0.0010553365498202035, 0.0011972457129536531, 0.0013286216753579422, 0.0014405792538840596, 0.0015238070380387695, 0.001569271028816185, 0.0015690150341873355, 0.001516981055392401, 0.0014097580972250404, 0.0012471643221047002, 0.0010325773968537242, 0.0007729513005631867, 0.00047849244520435603, 0.00016200920159745652, -0.00016200920159745196, -0.0004784924452043516, -0.0007729513005631823, -0.0010325773968537198, -0.0012471643221046963, -0.001409758097225037, -0.0015169810553923976, -0.0015690150341873324, -0.0015692710288161824, -0.0015238070380387673, -0.0014405792538840579, -0.0013286216753579405, -0.0011972457129536518, -0.0010553365498202022, -0.0009108005732581562, -0.0007701923879260784, -0.0006385249144990755, -0.000519245187788184, -0.00041434379264958243, -0.0003245581697577895],
            "type": "arbitrary",
        },
        "-5671910444091142186_q": {
            "samples": [0.0019126784932123073, 0.0025737902352024828, 0.0034097180757614314, 0.004447110734432495, 0.005710203213836212, 0.0072183732230314875, 0.008983410621423042, 0.011006704775378151, 0.013276620246507487, 0.015766376646515118, 0.018432761766696917, 0.021215978744641418, 0.02404085349478909, 0.026819510118246643, 0.02945546943320567, 0.031848956296730166, 0.033903037500186334, 0.03553007824122775, 0.036657923993314044] + [0.03723520229775363] * 2 + [0.036657923993314044, 0.03553007824122775, 0.033903037500186334, 0.031848956296730166, 0.02945546943320567, 0.026819510118246643, 0.02404085349478909, 0.021215978744641418, 0.018432761766696917, 0.015766376646515118, 0.013276620246507487, 0.011006704775378151, 0.008983410621423042, 0.0072183732230314875, 0.005710203213836212, 0.004447110734432495, 0.0034097180757614314, 0.0025737902352024828, 0.0019126784932123073],
            "type": "arbitrary",
        },
        "2330723448246942021_i": {
            "sample": 0.0033,
            "type": "constant",
        },
        "2330723448246942021_q": {
            "sample": 0.0,
            "type": "constant",
        },
        "-5270728341412406026": {
            "samples": [0.25] * 2 + [0.0] * 14,
            "type": "arbitrary",
        },
        "-6418550158589274799": {
            "samples": [0.25] * 3 + [0.0] * 13,
            "type": "arbitrary",
        },
        "-37518524385641680": {
            "samples": [0.25] * 4 + [0.0] * 12,
            "type": "arbitrary",
        },
        "-8619147853570476064": {
            "samples": [0.25] * 5 + [0.0] * 11,
            "type": "arbitrary",
        },
        "2160241163617525183": {
            "samples": [0.25] * 6 + [0.0] * 10,
            "type": "arbitrary",
        },
        "-192124505144769037": {
            "samples": [0.25] * 7 + [0.0] * 9,
            "type": "arbitrary",
        },
        "1233913405639371658": {
            "samples": [0.25] * 8 + [0.0] * 8,
            "type": "arbitrary",
        },
        "2005635182858397826": {
            "samples": [0.25] * 9 + [0.0] * 7,
            "type": "arbitrary",
        },
        "3431673093642538521": {
            "samples": [0.25] * 10 + [0.0] * 6,
            "type": "arbitrary",
        },
        "7238844999885162172": {
            "samples": [0.25] * 11 + [0.0] * 5,
            "type": "arbitrary",
        },
        "7460339059083877420": {
            "samples": [0.25] * 12 + [0.0] * 4,
            "type": "arbitrary",
        },
        "7183144655667875206": {
            "samples": [0.25] * 13 + [0.0] * 3,
            "type": "arbitrary",
        },
        "-8788645326622507333": {
            "samples": [0.25] * 14 + [0.0] * 2,
            "type": "arbitrary",
        },
        "1464247641275318685": {
            "samples": [0.25] * 15 + [0.0],
            "type": "arbitrary",
        },
        "-3555435509595742987": {
            "sample": 0.25,
            "type": "constant",
        },
        "966882260494260329": {
            "samples": [0.25] * 17 + [0.0] * 3,
            "type": "arbitrary",
        },
        "-8403705110227432949": {
            "samples": [0.25] * 18 + [0.0] * 2,
            "type": "arbitrary",
        },
        "-1136181762393860876": {
            "samples": [0.25] * 19 + [0.0],
            "type": "arbitrary",
        },
        "9116711205503965142": {
            "sample": 0.25,
            "type": "constant",
        },
        "1449356148982414773": {
            "samples": [0.25] * 21 + [0.0] * 3,
            "type": "arbitrary",
        },
        "-7132273180202419611": {
            "samples": [0.25] * 22 + [0.0] * 2,
            "type": "arbitrary",
        },
        "-751241545998786492": {
            "samples": [0.25] * 23 + [0.0],
            "type": "arbitrary",
        },
        "-1899063363175655265": {
            "sample": 0.25,
            "type": "constant",
        },
        "-1677569303976940017": {
            "samples": [0.25] * 25 + [0.0] * 3,
            "type": "arbitrary",
        },
        "-4002127386063776470": {
            "samples": [0.25] * 26 + [0.0] * 2,
            "type": "arbitrary",
        },
        "520190384026226846": {
            "samples": [0.25] * 27 + [0.0],
            "type": "arbitrary",
        },
        "-7147164672495323523": {
            "sample": 0.25,
            "type": "constant",
        },
        "-1292629087581865633": {
            "samples": [0.25] * 29 + [0.0] * 3,
            "type": "arbitrary",
        },
        "4271661946051563531": {
            "samples": [0.25] * 30 + [0.0] * 2,
            "type": "arbitrary",
        },
        "-5969552838430996306": {
            "samples": [0.25] * 31 + [0.0],
            "type": "arbitrary",
        },
        "1126624659620016478": {
            "sample": 0.25,
            "type": "constant",
        },
        "-2068560836227686534": {
            "samples": [0.25] * 33 + [0.0] * 3,
            "type": "arbitrary",
        },
        "-2345755239643688748": {
            "samples": [0.25] * 34 + [0.0] * 2,
            "type": "arbitrary",
        },
        "-2124261180444973500": {
            "samples": [0.25] * 35 + [0.0],
            "type": "arbitrary",
        },
        "8557594164649947687": {
            "sample": 0.25,
            "type": "constant",
        },
        "6205228495887653467": {
            "samples": [0.25] * 37 + [0.0] * 3,
            "type": "arbitrary",
        },
        "3880670413800817014": {
            "samples": [0.25] * 38 + [0.0] * 2,
            "type": "arbitrary",
        },
        "-1739320964049899116": {
            "samples": [0.25] * 39 + [0.0],
            "type": "arbitrary",
        },
        "735633127369269961": {
            "sample": 0.25,
            "type": "constant",
        },
        "2161671038153410656": {
            "samples": [0.25] * 41 + [0.0] * 3,
            "type": "arbitrary",
        },
        "-4589052013593251692": {
            "samples": [0.25] * 42 + [0.0] * 2,
            "type": "arbitrary",
        },
        "-6198464231832708480": {
            "samples": [0.25] * 43 + [0.0],
            "type": "arbitrary",
        },
        "-2391292325590084829": {
            "sample": 0.25,
            "type": "constant",
        },
        "8388096691597916418": {
            "samples": [0.25] * 45 + [0.0] * 3,
            "type": "arbitrary",
        },
        "7240274874421047645": {
            "samples": [0.25] * 46 + [0.0] * 2,
            "type": "arbitrary",
        },
        "5758536619419619984": {
            "samples": [0.25] * 47 + [0.0],
            "type": "arbitrary",
        },
        "5039677179439846380": {
            "sample": 0.25,
            "type": "constant",
        },
        "5261171238638561628": {
            "samples": [0.25] * 49 + [0.0] * 3,
            "type": "arbitrary",
        },
        "7625215090816122029": {
            "samples": [0.25] * 50 + [0.0] * 2,
            "type": "arbitrary",
        },
        "-5257237949260000423": {
            "samples": [0.25] * 51 + [0.0],
            "type": "arbitrary",
        },
        "-5035743890061285175": {
            "sample": 0.25,
            "type": "constant",
        },
        "-8402275235691547476": {
            "samples": [0.25] * 53 + [0.0] * 3,
            "type": "arbitrary",
        },
        "-8180781176492832228": {
            "samples": [0.25] * 54 + [0.0] * 2,
            "type": "arbitrary",
        },
        "3016551382855339578": {
            "samples": [0.25] * 55 + [0.0],
            "type": "arbitrary",
        },
        "2395225614968646034": {
            "sample": 0.25,
            "type": "constant",
        },
        "2616719674167361282": {
            "samples": [0.25] * 57 + [0.0] * 3,
            "type": "arbitrary",
        },
        "-749811671462901019": {
            "samples": [0.25] * 58 + [0.0] * 2,
            "type": "arbitrary",
        },
        "4814479362170528145": {
            "samples": [0.25] * 59 + [0.0],
            "type": "arbitrary",
        },
        "-3379371743641197453": {
            "sample": 0.25,
            "type": "constant",
        },
        "3001659890562435666": {
            "samples": [0.25] * 61 + [0.0] * 3,
            "type": "arbitrary",
        },
        "-3876737124422255809": {
            "samples": [0.25] * 62 + [0.0] * 2,
            "type": "arbitrary",
        },
        "6902651892765745438": {
            "samples": [0.25] * 63 + [0.0],
            "type": "arbitrary",
        },
        "2847053909803308309": {
            "sample": 0.25,
            "type": "constant",
        },
        "4273091820587449004": {
            "samples": [0.25] * 65 + [0.0] * 3,
            "type": "arbitrary",
        },
        "6748045912006618081": {
            "samples": [0.25] * 66 + [0.0] * 2,
            "type": "arbitrary",
        },
        "1128054534155901951": {
            "samples": [0.25] * 67 + [0.0],
            "type": "arbitrary",
        },
        "-8168720658876312098": {
            "sample": 0.25,
            "type": "constant",
        },
        "-1072543160825299314": {
            "samples": [0.25] * 69 + [0.0] * 3,
            "type": "arbitrary",
        },
        "-4267728656673002326": {
            "samples": [0.25] * 70 + [0.0] * 2,
            "type": "arbitrary",
        },
        "-4046234597474287078": {
            "samples": [0.25] * 71 + [0.0],
            "type": "arbitrary",
        },
        "-4323429000890289292": {
            "sample": 0.25,
            "type": "constant",
        },
        "-516257094647665641": {
            "samples": [0.25] * 73 + [0.0] * 3,
            "type": "arbitrary",
        },
        "4006060675442337675": {
            "samples": [0.25] * 74 + [0.0] * 2,
            "type": "arbitrary",
        },
        "-3661294381079212694": {
            "samples": [0.25] * 75 + [0.0],
            "type": "arbitrary",
        },
        "3283707827749389633": {
            "sample": 0.25,
            "type": "constant",
        },
        "-7133674280342917920": {
            "samples": [0.25] * 77 + [0.0] * 3,
            "type": "arbitrary",
        },
        "-6912180221144202672": {
            "samples": [0.25] * 78 + [0.0] * 2,
            "type": "arbitrary",
        },
        "-2389862451054199356": {
            "samples": [0.25] * 79 + [0.0],
            "type": "arbitrary",
        },
        "3274127046900791126_i": {
            "samples": [0.0011180555187915678, 0.0015045081475491685, 0.0019931494632565378, 0.0025995569652476707, 0.0033378972155056613, 0.004219497446825634, 0.005251249417242201, 0.006433965280360931, 0.007760843544895696, 0.009216229744590979, 0.010774864198537374, 0.012401792672522064, 0.01405307218212388, 0.015677334902548905, 0.0172181839630987, 0.018617295840194316, 0.019818008261921365, 0.020769094336384877, 0.021428376161292048] + [0.02176582398456596] * 2 + [0.021428376161292048, 0.020769094336384877, 0.019818008261921365, 0.018617295840194316, 0.0172181839630987, 0.015677334902548905, 0.01405307218212388, 0.012401792672522064, 0.010774864198537374, 0.009216229744590979, 0.007760843544895696, 0.006433965280360931, 0.005251249417242201, 0.004219497446825634, 0.0033378972155056613, 0.0025995569652476707, 0.0019931494632565378, 0.0015045081475491685, 0.0011180555187915678],
            "type": "arbitrary",
        },
        "3274127046900791126_q": {
            "samples": [0.0002214274015731738, 0.0002826829761606055, 0.00035425117413348626, 0.00043562888206689486, 0.0005254580382221803, 0.0006213869286926813, 0.0007199955255671924, 0.0008168119984833213, 0.0009064422734077396, 0.0009828245001067724, 0.0010396060379057545, 0.001070623508057569, 0.0010704488576227173, 0.0010349490619260802, 0.0009617969948137164, 0.000850868598946381, 0.000704468262435486, 0.0005273402858847722, 0.0003264479180821836, 0.00011052957492162402, -0.00011052957492162402, -0.0003264479180821836, -0.0005273402858847722, -0.000704468262435486, -0.000850868598946381, -0.0009617969948137164, -0.0010349490619260802, -0.0010704488576227173, -0.001070623508057569, -0.0010396060379057545, -0.0009828245001067724, -0.0009064422734077396, -0.0008168119984833213, -0.0007199955255671924, -0.0006213869286926813, -0.0005254580382221803, -0.00043562888206689486, -0.00035425117413348626, -0.0002826829761606055, -0.0002214274015731738],
            "type": "arbitrary",
        },
        "-8503795126481802350_i": {
            "samples": [0.0012493039327485741, 0.0016811221929452756, 0.0022271250587747485, 0.0029047186704985986, 0.0037297326012476254, 0.004714823786424647, 0.00586769300677935, 0.007189247754539428, 0.008671888112107319, 0.010298121937161474, 0.01203972431763831, 0.013857637745628482, 0.01570276077461938, 0.017517695516533722, 0.019239424678246013, 0.020802778144181227, 0.022144442064578468, 0.023207176028356858, 0.023943850873928398] + [0.024320911659934084] * 2 + [0.023943850873928398, 0.023207176028356858, 0.022144442064578468, 0.020802778144181227, 0.019239424678246013, 0.017517695516533722, 0.01570276077461938, 0.013857637745628482, 0.01203972431763831, 0.010298121937161474, 0.008671888112107319, 0.007189247754539428, 0.00586769300677935, 0.004714823786424647, 0.0037297326012476254, 0.0029047186704985986, 0.0022271250587747485, 0.0016811221929452756, 0.0012493039327485741],
            "type": "arbitrary",
        },
        "-8503795126481802350_q": {
            "samples": [-0.00018478027949113372, -0.0002358978110714118, -0.0002956211855505884, -0.0003635305567912292, -0.00043849262771333716, -0.0005185449024836564, -0.0006008333815122076, -0.0006816263402774133, -0.0007564224456091558, -0.0008201631077734918, -0.0008675470735784872, -0.000893431028152493, -0.0008932852830643251, -0.000863660845781051, -0.0008026157388505532, -0.0007100464369202509, -0.0005878759426368707, -0.0004400633558467762, -0.00027241947976610573, -9.223648744893299e-05, 9.223648744893299e-05, 0.00027241947976610573, 0.0004400633558467762, 0.0005878759426368707, 0.0007100464369202509, 0.0008026157388505532, 0.000863660845781051, 0.0008932852830643251, 0.000893431028152493, 0.0008675470735784872, 0.0008201631077734918, 0.0007564224456091558, 0.0006816263402774133, 0.0006008333815122076, 0.0005185449024836564, 0.00043849262771333716, 0.0003635305567912292, 0.0002956211855505884, 0.0002358978110714118, 0.00018478027949113372],
            "type": "arbitrary",
        },
        "-2818334305619283106_i": {
            "samples": [0.0016409510779196645, 0.0022081410314293824, 0.002925311583561483, 0.00381532556527005, 0.004898974998745269, 0.006192884670996909, 0.007707169498132072, 0.009443021464190989, 0.011390458136038771, 0.01352650371967906, 0.015814085010857203, 0.018201900274243128, 0.02062545513863524, 0.023009357920839617, 0.025270835892482323, 0.02732428860950993, 0.029086553818695706, 0.030482446681665623, 0.031450063408251405] + [0.03194532984183734] * 2 + [0.031450063408251405, 0.030482446681665623, 0.029086553818695706, 0.02732428860950993, 0.025270835892482323, 0.023009357920839617, 0.02062545513863524, 0.018201900274243128, 0.015814085010857203, 0.01352650371967906, 0.011390458136038771, 0.009443021464190989, 0.007707169498132072, 0.006192884670996909, 0.004898974998745269, 0.00381532556527005, 0.002925311583561483, 0.0022081410314293824, 0.0016409510779196645],
            "type": "arbitrary",
        },
        "-2818334305619283106_q": {
            "samples": [-0.00030171729028092745, -0.00038518422277353297, -0.00048270315046383824, -0.0005935885302881004, -0.0007159898654021668, -0.0008467027071136326, -0.0009810669205578746, -0.001112989183363989, -0.0012351195226318382, -0.0013391980526670647, -0.0014165686563095024, -0.001458833104968908, -0.0014585951260395822, -0.001410223054258862, -0.001310545944240944, -0.0011593947552800683, -0.000959909449873158, -0.000718554618727004, -0.0004448183944797706, -0.0001506077549766785, 0.0001506077549766785, 0.0004448183944797706, 0.000718554618727004, 0.000959909449873158, 0.0011593947552800683, 0.001310545944240944, 0.001410223054258862, 0.0014585951260395822, 0.001458833104968908, 0.0014165686563095024, 0.0013391980526670647, 0.0012351195226318382, 0.001112989183363989, 0.0009810669205578746, 0.0008467027071136326, 0.0007159898654021668, 0.0005935885302881004, 0.00048270315046383824, 0.00038518422277353297, 0.00030171729028092745],
            "type": "arbitrary",
        },
        "-4910819814867580317_i": {
            "samples": [0.0019126784932123073, 0.0025737902352024828, 0.0034097180757614314, 0.004447110734432495, 0.005710203213836212, 0.0072183732230314875, 0.008983410621423042, 0.011006704775378151, 0.013276620246507487, 0.015766376646515118, 0.018432761766696917, 0.021215978744641418, 0.02404085349478909, 0.026819510118246643, 0.02945546943320567, 0.031848956296730166, 0.033903037500186334, 0.03553007824122775, 0.036657923993314044] + [0.03723520229775363] * 2 + [0.036657923993314044, 0.03553007824122775, 0.033903037500186334, 0.031848956296730166, 0.02945546943320567, 0.026819510118246643, 0.02404085349478909, 0.021215978744641418, 0.018432761766696917, 0.015766376646515118, 0.013276620246507487, 0.011006704775378151, 0.008983410621423042, 0.0072183732230314875, 0.005710203213836212, 0.004447110734432495, 0.0034097180757614314, 0.0025737902352024828, 0.0019126784932123073],
            "type": "arbitrary",
        },
        "-4910819814867580317_q": {
            "samples": [-0.0003245581697577896, -0.0004143437926495826, -0.0005192451877881842, -0.0006385249144990759, -0.0007701923879260787, -0.0009108005732581567, -0.0010553365498202029, -0.0011972457129536525, -0.0013286216753579413, -0.0014405792538840587, -0.0015238070380387684, -0.0015692710288161837, -0.001569015034187334, -0.0015169810553923994, -0.0014097580972250387, -0.0012471643221046982, -0.001032577396853722, -0.0007729513005631845, -0.0004784924452043538, -0.00016200920159745424, 0.00016200920159745424, 0.0004784924452043538, 0.0007729513005631845, 0.001032577396853722, 0.0012471643221046982, 0.0014097580972250387, 0.0015169810553923994, 0.001569015034187334, 0.0015692710288161837, 0.0015238070380387684, 0.0014405792538840587, 0.0013286216753579413, 0.0011972457129536525, 0.0010553365498202029, 0.0009108005732581567, 0.0007701923879260787, 0.0006385249144990759, 0.0005192451877881842, 0.0004143437926495826, 0.0003245581697577896],
            "type": "arbitrary",
        },
    },
    "digital_waveforms": {
        "ON": {
            "samples": [(1, 0)],
        },
    },
    "integration_weights": {
        "cosine_weights_B1/acquisition": {
            "cosine": [(1.0, 2000.0)],
            "sine": [(-0.0, 2000.0)],
        },
        "sine_weights_B1/acquisition": {
            "cosine": [(0.0, 2000.0)],
            "sine": [(1.0, 2000.0)],
        },
        "minus_sine_weights_B1/acquisition": {
            "cosine": [(-0.0, 2000.0)],
            "sine": [(-1.0, 2000.0)],
        },
        "cosine_weights_B2/acquisition": {
            "cosine": [(1.0, 2000.0)],
            "sine": [(-0.0, 2000.0)],
        },
        "sine_weights_B2/acquisition": {
            "cosine": [(0.0, 2000.0)],
            "sine": [(1.0, 2000.0)],
        },
        "minus_sine_weights_B2/acquisition": {
            "cosine": [(-0.0, 2000.0)],
            "sine": [(-1.0, 2000.0)],
        },
        "cosine_weights_B3/acquisition": {
            "cosine": [(1.0, 2000.0)],
            "sine": [(-0.0, 2000.0)],
        },
        "sine_weights_B3/acquisition": {
            "cosine": [(0.0, 2000.0)],
            "sine": [(1.0, 2000.0)],
        },
        "minus_sine_weights_B3/acquisition": {
            "cosine": [(-0.0, 2000.0)],
            "sine": [(-1.0, 2000.0)],
        },
        "cosine_weights_B4/acquisition": {
            "cosine": [(1.0, 2000.0)],
            "sine": [(-0.0, 2000.0)],
        },
        "sine_weights_B4/acquisition": {
            "cosine": [(0.0, 2000.0)],
            "sine": [(1.0, 2000.0)],
        },
        "minus_sine_weights_B4/acquisition": {
            "cosine": [(-0.0, 2000.0)],
            "sine": [(-1.0, 2000.0)],
        },
    },
    "mixers": {},
}

loaded_config = {
    "version": 1,
    "controllers": {
        "con4": {
            "type": "opx1",
            "analog_outputs": {
                "1": {
                    "offset": -0.25498290735703955,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
                "2": {
                    "offset": 0.10729465913983083,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
                "3": {
                    "offset": 0.2226188560782865,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
                "4": {
                    "offset": 0.114743772754262,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
                "5": {
                    "offset": 0.0,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
            },
            "analog_inputs": {
                "1": {
                    "offset": 0.0,
                    "gain_db": 0,
                    "shareable": False,
                    "sampling_rate": 1000000000.0,
                },
                "2": {
                    "offset": 0.0,
                    "gain_db": 0,
                    "shareable": False,
                    "sampling_rate": 1000000000.0,
                },
            },
            "digital_outputs": {},
            "digital_inputs": {},
        },
        "con9": {
            "type": "opx1",
            "analog_outputs": {
                "3": {
                    "offset": -0.4530870218982913,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
                "4": {
                    "offset": -0.2224873138863394,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [1.0891790415038731, -1.024484298837039],
                        "feedback": [0.935305257333166],
                    },
                    "crosstalk": {},
                },
                "5": {
                    "offset": 0.05128942239382605,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [1.1298143371682787, -0.9007185757251136],
                        "feedback": [0.7709042385568349],
                    },
                    "crosstalk": {},
                },
                "6": {
                    "offset": -0.08416990010352905,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
                "7": {
                    "offset": 0.05418914676504196,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
            },
            "analog_inputs": {
                "1": {
                    "offset": 0.0,
                    "gain_db": 0,
                    "shareable": False,
                    "sampling_rate": 1000000000.0,
                },
                "2": {
                    "offset": 0.0,
                    "gain_db": 0,
                    "shareable": False,
                    "sampling_rate": 1000000000.0,
                },
            },
            "digital_outputs": {},
            "digital_inputs": {},
        },
        "con2": {
            "type": "opx1",
            "analog_outputs": {
                "3": {
                    "offset": 0.0,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
                "4": {
                    "offset": 0.0,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
                "1": {
                    "offset": 0.0,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
                "2": {
                    "offset": 0.0,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
                "7": {
                    "offset": 0.0,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
                "8": {
                    "offset": 0.0,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
            },
            "analog_inputs": {
                "1": {
                    "offset": 0.0,
                    "gain_db": 10,
                    "shareable": False,
                    "sampling_rate": 1000000000.0,
                },
                "2": {
                    "offset": 0.0,
                    "gain_db": 10,
                    "shareable": False,
                    "sampling_rate": 1000000000.0,
                },
            },
            "digital_outputs": {
                "3": {
                    "shareable": False,
                    "inverted": False,
                    "level": "LVTTL",
                },
                "1": {
                    "shareable": False,
                    "inverted": False,
                    "level": "LVTTL",
                },
                "7": {
                    "shareable": False,
                    "inverted": False,
                    "level": "LVTTL",
                },
            },
            "digital_inputs": {},
        },
        "con3": {
            "type": "opx1",
            "analog_outputs": {
                "1": {
                    "offset": 0.0,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
                "2": {
                    "offset": 0.0,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
                "7": {
                    "offset": 0.0,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
                "8": {
                    "offset": 0.0,
                    "delay": 0,
                    "shareable": False,
                    "filter": {
                        "feedforward": [],
                        "feedback": [],
                    },
                    "crosstalk": {},
                },
            },
            "analog_inputs": {
                "1": {
                    "offset": 0.0,
                    "gain_db": 0,
                    "shareable": False,
                    "sampling_rate": 1000000000.0,
                },
                "2": {
                    "offset": 0.0,
                    "gain_db": 0,
                    "shareable": False,
                    "sampling_rate": 1000000000.0,
                },
            },
            "digital_outputs": {
                "1": {
                    "shareable": False,
                    "inverted": False,
                    "level": "LVTTL",
                },
                "7": {
                    "shareable": False,
                    "inverted": False,
                    "level": "LVTTL",
                },
            },
            "digital_inputs": {},
        },
    },
    "oscillators": {},
    "elements": {
        "B1/flux": {
            "digitalInputs": {},
            "digitalOutputs": {},
            "outputs": {},
            "operations": {
                "-6912180221144202672": "-6912180221144202672",
                "-2389862451054199356": "-2389862451054199356",
            },
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "singleInput": {
                "port": ('con4', 1, 1),
            },
            "intermediate_frequency": 0,
        },
        "D1/flux": {
            "digitalInputs": {},
            "digitalOutputs": {},
            "outputs": {},
            "operations": {},
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "singleInput": {
                "port": ('con9', 1, 3),
            },
            "intermediate_frequency": 0,
        },
        "B2/flux": {
            "digitalInputs": {},
            "digitalOutputs": {},
            "outputs": {},
            "operations": {
                "-6912180221144202672": "-6912180221144202672",
                "-2389862451054199356": "-2389862451054199356",
            },
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "singleInput": {
                "port": ('con4', 1, 2),
            },
            "intermediate_frequency": 0,
        },
        "D2/flux": {
            "digitalInputs": {},
            "digitalOutputs": {},
            "outputs": {},
            "operations": {},
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "singleInput": {
                "port": ('con9', 1, 4),
            },
            "intermediate_frequency": 0,
        },
        "B3/flux": {
            "digitalInputs": {},
            "digitalOutputs": {},
            "outputs": {},
            "operations": {
                "-6912180221144202672": "-6912180221144202672",
                "-2389862451054199356": "-2389862451054199356",
            },
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "singleInput": {
                "port": ('con4', 1, 3),
            },
            "intermediate_frequency": 0,
        },
        "D3/flux": {
            "digitalInputs": {},
            "digitalOutputs": {},
            "outputs": {},
            "operations": {},
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "singleInput": {
                "port": ('con9', 1, 5),
            },
            "intermediate_frequency": 0,
        },
        "B4/flux": {
            "digitalInputs": {},
            "digitalOutputs": {},
            "outputs": {},
            "operations": {
                "-6912180221144202672": "-6912180221144202672",
                "-2389862451054199356": "-2389862451054199356",
            },
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "singleInput": {
                "port": ('con4', 1, 4),
            },
            "intermediate_frequency": 0,
        },
        "D4/flux": {
            "digitalInputs": {},
            "digitalOutputs": {},
            "outputs": {},
            "operations": {},
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "singleInput": {
                "port": ('con9', 1, 6),
            },
            "intermediate_frequency": 0,
        },
        "B5/flux": {
            "digitalInputs": {},
            "digitalOutputs": {},
            "outputs": {},
            "operations": {},
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "singleInput": {
                "port": ('con4', 1, 5),
            },
            "intermediate_frequency": 0,
        },
        "D5/flux": {
            "digitalInputs": {},
            "digitalOutputs": {},
            "outputs": {},
            "operations": {},
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "singleInput": {
                "port": ('con9', 1, 7),
            },
            "intermediate_frequency": 0,
        },
        "B1/drive": {
            "digitalInputs": {
                "output_switch": {
                    "delay": 57,
                    "buffer": 18,
                    "port": ('con2', 1, 3),
                },
            },
            "digitalOutputs": {},
            "outputs": {},
            "operations": {
                "-1885320965307138871": "-1885320965307138871",
                "3274127046900791126": "3274127046900791126",
            },
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "mixInputs": {
                "I": ('con2', 1, 3),
                "Q": ('con2', 1, 4),
                "mixer": "B1/drive_mixer_288",
                "lo_frequency": 4900000000.0,
            },
            "intermediate_frequency": 100388701.0,
        },
        "B4/acquisition": {
            "digitalInputs": {
                "output_switch": {
                    "delay": 57,
                    "buffer": 18,
                    "port": ('con2', 1, 1),
                },
            },
            "digitalOutputs": {},
            "outputs": {
                "out1": ('con2', 1, 1),
                "out2": ('con2', 1, 2),
            },
            "operations": {
                "2690639089030047106": "2690639089030047106_B4/acquisition",
            },
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "mixInputs": {
                "I": ('con2', 1, 1),
                "Q": ('con2', 1, 2),
                "mixer": "B4/acquisition_mixer_9da",
                "lo_frequency": 7370000000.0,
            },
            "smearing": 0,
            "time_of_flight": 224,
            "intermediate_frequency": 330300527.0,
        },
        "B3/acquisition": {
            "digitalInputs": {
                "output_switch": {
                    "delay": 57,
                    "buffer": 18,
                    "port": ('con2', 1, 1),
                },
            },
            "digitalOutputs": {},
            "outputs": {
                "out1": ('con2', 1, 1),
                "out2": ('con2', 1, 2),
            },
            "operations": {
                "-4041517210051566932": "-4041517210051566932_B3/acquisition",
            },
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "mixInputs": {
                "I": ('con2', 1, 1),
                "Q": ('con2', 1, 2),
                "mixer": "B3/acquisition_mixer_9a4",
                "lo_frequency": 7370000000.0,
            },
            "smearing": 0,
            "time_of_flight": 224,
            "intermediate_frequency": 110622376.0,
        },
        "B3/drive": {
            "digitalInputs": {
                "output_switch": {
                    "delay": 57,
                    "buffer": 18,
                    "port": ('con3', 1, 1),
                },
            },
            "digitalOutputs": {},
            "outputs": {},
            "operations": {
                "-3579424934842844975": "-3579424934842844975",
                "-2818334305619283106": "-2818334305619283106",
            },
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "mixInputs": {
                "I": ('con3', 1, 1),
                "Q": ('con3', 1, 2),
                "mixer": "B3/drive_mixer_223",
                "lo_frequency": 5800000000.0,
            },
            "intermediate_frequency": -115376210.0,
        },
        "B1/acquisition": {
            "digitalInputs": {
                "output_switch": {
                    "delay": 57,
                    "buffer": 18,
                    "port": ('con2', 1, 1),
                },
            },
            "digitalOutputs": {},
            "outputs": {
                "out1": ('con2', 1, 1),
                "out2": ('con2', 1, 2),
            },
            "operations": {
                "-7269982027912257413": "-7269982027912257413_B1/acquisition",
            },
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "mixInputs": {
                "I": ('con2', 1, 1),
                "Q": ('con2', 1, 2),
                "mixer": "B1/acquisition_mixer_db6",
                "lo_frequency": 7370000000.0,
            },
            "smearing": 0,
            "time_of_flight": 224,
            "intermediate_frequency": -237451236.0,
        },
        "B4/drive": {
            "digitalInputs": {
                "output_switch": {
                    "delay": 57,
                    "buffer": 18,
                    "port": ('con3', 1, 7),
                },
            },
            "digitalOutputs": {},
            "outputs": {},
            "operations": {
                "-5671910444091142186": "-5671910444091142186",
                "-4910819814867580317": "-4910819814867580317",
            },
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "mixInputs": {
                "I": ('con3', 1, 7),
                "Q": ('con3', 1, 8),
                "mixer": "B4/drive_mixer_7be",
                "lo_frequency": 6700000000.0,
            },
            "intermediate_frequency": 109615374.0,
        },
        "B2/drive": {
            "digitalInputs": {
                "output_switch": {
                    "delay": 57,
                    "buffer": 18,
                    "port": ('con2', 1, 7),
                },
            },
            "digitalOutputs": {},
            "outputs": {},
            "operations": {
                "9181858318004187397": "9181858318004187397",
                "-8503795126481802350": "-8503795126481802350",
            },
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "mixInputs": {
                "I": ('con2', 1, 7),
                "Q": ('con2', 1, 8),
                "mixer": "B2/drive_mixer_3c3",
                "lo_frequency": 5900000000.0,
            },
            "intermediate_frequency": 63761228.0,
        },
        "B2/acquisition": {
            "digitalInputs": {
                "output_switch": {
                    "delay": 57,
                    "buffer": 18,
                    "port": ('con2', 1, 1),
                },
            },
            "digitalOutputs": {},
            "outputs": {
                "out1": ('con2', 1, 1),
                "out2": ('con2', 1, 2),
            },
            "operations": {
                "-9087730669898779636": "-9087730669898779636_B2/acquisition",
            },
            "hold_offset": {
                "duration": 0,
            },
            "sticky": {
                "analog": False,
                "digital": False,
                "duration": 4,
            },
            "thread": "",
            "mixInputs": {
                "I": ('con2', 1, 1),
                "Q": ('con2', 1, 2),
                "mixer": "B2/acquisition_mixer_5fd",
                "lo_frequency": 7370000000.0,
            },
            "smearing": 0,
            "time_of_flight": 224,
            "intermediate_frequency": 10040944.0,
        },
    },
    "pulses": {
        "-1885320965307138871": {
            "length": 40,
            "waveforms": {
                "I": "-1885320965307138871_i",
                "Q": "-1885320965307138871_q",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-5492222400611121274": {
            "length": 16,
            "waveforms": {
                "single": "-5492222400611121274",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-7269982027912257413_B1/acquisition": {
            "length": 2000,
            "waveforms": {
                "I": "-3597628208875341953_i",
                "Q": "-3597628208875341953_q",
            },
            "integration_weights": {
                "cos": "cosine_weights_B1/acquisition",
                "sin": "sine_weights_B1/acquisition",
                "minus_sin": "minus_sine_weights_B1/acquisition",
            },
            "operation": "measurement",
            "digital_marker": "ON",
        },
        "9181858318004187397": {
            "length": 40,
            "waveforms": {
                "I": "9181858318004187397_i",
                "Q": "9181858318004187397_q",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-9087730669898779636_B2/acquisition": {
            "length": 2000,
            "waveforms": {
                "I": "191989489022899436_i",
                "Q": "191989489022899436_q",
            },
            "integration_weights": {
                "cos": "cosine_weights_B2/acquisition",
                "sin": "sine_weights_B2/acquisition",
                "minus_sin": "minus_sine_weights_B2/acquisition",
            },
            "operation": "measurement",
            "digital_marker": "ON",
        },
        "-3579424934842844975": {
            "length": 40,
            "waveforms": {
                "I": "-3579424934842844975_i",
                "Q": "-3579424934842844975_q",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-4041517210051566932_B3/acquisition": {
            "length": 2000,
            "waveforms": {
                "I": "-676466526619785515_i",
                "Q": "-676466526619785515_q",
            },
            "integration_weights": {
                "cos": "cosine_weights_B3/acquisition",
                "sin": "sine_weights_B3/acquisition",
                "minus_sin": "minus_sine_weights_B3/acquisition",
            },
            "operation": "measurement",
            "digital_marker": "ON",
        },
        "-5671910444091142186": {
            "length": 40,
            "waveforms": {
                "I": "-5671910444091142186_i",
                "Q": "-5671910444091142186_q",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "2690639089030047106_B4/acquisition": {
            "length": 2000,
            "waveforms": {
                "I": "2330723448246942021_i",
                "Q": "2330723448246942021_q",
            },
            "integration_weights": {
                "cos": "cosine_weights_B4/acquisition",
                "sin": "sine_weights_B4/acquisition",
                "minus_sin": "minus_sine_weights_B4/acquisition",
            },
            "operation": "measurement",
            "digital_marker": "ON",
        },
        "-5270728341412406026": {
            "length": 16,
            "waveforms": {
                "single": "-5270728341412406026",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-6418550158589274799": {
            "length": 16,
            "waveforms": {
                "single": "-6418550158589274799",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-37518524385641680": {
            "length": 16,
            "waveforms": {
                "single": "-37518524385641680",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-8619147853570476064": {
            "length": 16,
            "waveforms": {
                "single": "-8619147853570476064",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "2160241163617525183": {
            "length": 16,
            "waveforms": {
                "single": "2160241163617525183",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-192124505144769037": {
            "length": 16,
            "waveforms": {
                "single": "-192124505144769037",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "1233913405639371658": {
            "length": 16,
            "waveforms": {
                "single": "1233913405639371658",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "2005635182858397826": {
            "length": 16,
            "waveforms": {
                "single": "2005635182858397826",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "3431673093642538521": {
            "length": 16,
            "waveforms": {
                "single": "3431673093642538521",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "7238844999885162172": {
            "length": 16,
            "waveforms": {
                "single": "7238844999885162172",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "7460339059083877420": {
            "length": 16,
            "waveforms": {
                "single": "7460339059083877420",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "7183144655667875206": {
            "length": 16,
            "waveforms": {
                "single": "7183144655667875206",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-8788645326622507333": {
            "length": 16,
            "waveforms": {
                "single": "-8788645326622507333",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "1464247641275318685": {
            "length": 16,
            "waveforms": {
                "single": "1464247641275318685",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-3555435509595742987": {
            "length": 16,
            "waveforms": {
                "single": "-3555435509595742987",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "966882260494260329": {
            "length": 20,
            "waveforms": {
                "single": "966882260494260329",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-8403705110227432949": {
            "length": 20,
            "waveforms": {
                "single": "-8403705110227432949",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-1136181762393860876": {
            "length": 20,
            "waveforms": {
                "single": "-1136181762393860876",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "9116711205503965142": {
            "length": 20,
            "waveforms": {
                "single": "9116711205503965142",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "1449356148982414773": {
            "length": 24,
            "waveforms": {
                "single": "1449356148982414773",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-7132273180202419611": {
            "length": 24,
            "waveforms": {
                "single": "-7132273180202419611",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-751241545998786492": {
            "length": 24,
            "waveforms": {
                "single": "-751241545998786492",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-1899063363175655265": {
            "length": 24,
            "waveforms": {
                "single": "-1899063363175655265",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-1677569303976940017": {
            "length": 28,
            "waveforms": {
                "single": "-1677569303976940017",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-4002127386063776470": {
            "length": 28,
            "waveforms": {
                "single": "-4002127386063776470",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "520190384026226846": {
            "length": 28,
            "waveforms": {
                "single": "520190384026226846",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-7147164672495323523": {
            "length": 28,
            "waveforms": {
                "single": "-7147164672495323523",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-1292629087581865633": {
            "length": 32,
            "waveforms": {
                "single": "-1292629087581865633",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "4271661946051563531": {
            "length": 32,
            "waveforms": {
                "single": "4271661946051563531",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-5969552838430996306": {
            "length": 32,
            "waveforms": {
                "single": "-5969552838430996306",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "1126624659620016478": {
            "length": 32,
            "waveforms": {
                "single": "1126624659620016478",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-2068560836227686534": {
            "length": 36,
            "waveforms": {
                "single": "-2068560836227686534",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-2345755239643688748": {
            "length": 36,
            "waveforms": {
                "single": "-2345755239643688748",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-2124261180444973500": {
            "length": 36,
            "waveforms": {
                "single": "-2124261180444973500",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "8557594164649947687": {
            "length": 36,
            "waveforms": {
                "single": "8557594164649947687",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "6205228495887653467": {
            "length": 40,
            "waveforms": {
                "single": "6205228495887653467",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "3880670413800817014": {
            "length": 40,
            "waveforms": {
                "single": "3880670413800817014",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-1739320964049899116": {
            "length": 40,
            "waveforms": {
                "single": "-1739320964049899116",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "735633127369269961": {
            "length": 40,
            "waveforms": {
                "single": "735633127369269961",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "2161671038153410656": {
            "length": 44,
            "waveforms": {
                "single": "2161671038153410656",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-4589052013593251692": {
            "length": 44,
            "waveforms": {
                "single": "-4589052013593251692",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-6198464231832708480": {
            "length": 44,
            "waveforms": {
                "single": "-6198464231832708480",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-2391292325590084829": {
            "length": 44,
            "waveforms": {
                "single": "-2391292325590084829",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "8388096691597916418": {
            "length": 48,
            "waveforms": {
                "single": "8388096691597916418",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "7240274874421047645": {
            "length": 48,
            "waveforms": {
                "single": "7240274874421047645",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "5758536619419619984": {
            "length": 48,
            "waveforms": {
                "single": "5758536619419619984",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "5039677179439846380": {
            "length": 48,
            "waveforms": {
                "single": "5039677179439846380",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "5261171238638561628": {
            "length": 52,
            "waveforms": {
                "single": "5261171238638561628",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "7625215090816122029": {
            "length": 52,
            "waveforms": {
                "single": "7625215090816122029",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-5257237949260000423": {
            "length": 52,
            "waveforms": {
                "single": "-5257237949260000423",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-5035743890061285175": {
            "length": 52,
            "waveforms": {
                "single": "-5035743890061285175",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-8402275235691547476": {
            "length": 56,
            "waveforms": {
                "single": "-8402275235691547476",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-8180781176492832228": {
            "length": 56,
            "waveforms": {
                "single": "-8180781176492832228",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "3016551382855339578": {
            "length": 56,
            "waveforms": {
                "single": "3016551382855339578",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "2395225614968646034": {
            "length": 56,
            "waveforms": {
                "single": "2395225614968646034",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "2616719674167361282": {
            "length": 60,
            "waveforms": {
                "single": "2616719674167361282",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-749811671462901019": {
            "length": 60,
            "waveforms": {
                "single": "-749811671462901019",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "4814479362170528145": {
            "length": 60,
            "waveforms": {
                "single": "4814479362170528145",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-3379371743641197453": {
            "length": 60,
            "waveforms": {
                "single": "-3379371743641197453",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "3001659890562435666": {
            "length": 64,
            "waveforms": {
                "single": "3001659890562435666",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-3876737124422255809": {
            "length": 64,
            "waveforms": {
                "single": "-3876737124422255809",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "6902651892765745438": {
            "length": 64,
            "waveforms": {
                "single": "6902651892765745438",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "2847053909803308309": {
            "length": 64,
            "waveforms": {
                "single": "2847053909803308309",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "4273091820587449004": {
            "length": 68,
            "waveforms": {
                "single": "4273091820587449004",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "6748045912006618081": {
            "length": 68,
            "waveforms": {
                "single": "6748045912006618081",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "1128054534155901951": {
            "length": 68,
            "waveforms": {
                "single": "1128054534155901951",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-8168720658876312098": {
            "length": 68,
            "waveforms": {
                "single": "-8168720658876312098",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-1072543160825299314": {
            "length": 72,
            "waveforms": {
                "single": "-1072543160825299314",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-4267728656673002326": {
            "length": 72,
            "waveforms": {
                "single": "-4267728656673002326",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-4046234597474287078": {
            "length": 72,
            "waveforms": {
                "single": "-4046234597474287078",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-4323429000890289292": {
            "length": 72,
            "waveforms": {
                "single": "-4323429000890289292",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-516257094647665641": {
            "length": 76,
            "waveforms": {
                "single": "-516257094647665641",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "4006060675442337675": {
            "length": 76,
            "waveforms": {
                "single": "4006060675442337675",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-3661294381079212694": {
            "length": 76,
            "waveforms": {
                "single": "-3661294381079212694",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "3283707827749389633": {
            "length": 76,
            "waveforms": {
                "single": "3283707827749389633",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-7133674280342917920": {
            "length": 80,
            "waveforms": {
                "single": "-7133674280342917920",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-6912180221144202672": {
            "length": 80,
            "waveforms": {
                "single": "-6912180221144202672",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-2389862451054199356": {
            "length": 80,
            "waveforms": {
                "single": "-2389862451054199356",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "3274127046900791126": {
            "length": 40,
            "waveforms": {
                "I": "3274127046900791126_i",
                "Q": "3274127046900791126_q",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-8503795126481802350": {
            "length": 40,
            "waveforms": {
                "I": "-8503795126481802350_i",
                "Q": "-8503795126481802350_q",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-2818334305619283106": {
            "length": 40,
            "waveforms": {
                "I": "-2818334305619283106_i",
                "Q": "-2818334305619283106_q",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
        "-4910819814867580317": {
            "length": 40,
            "waveforms": {
                "I": "-4910819814867580317_i",
                "Q": "-4910819814867580317_q",
            },
            "integration_weights": {},
            "operation": "control",
            "digital_marker": "ON",
        },
    },
    "waveforms": {
        "-1885320965307138871_i": {
            "type": "arbitrary",
            "samples": [-0.00022142740157317373, -0.0002826829761606054, -0.00035425117413348615, -0.0004356288820668947, -0.0005254580382221801, -0.0006213869286926811, -0.0007199955255671921, -0.0008168119984833208, -0.0009064422734077392, -0.0009828245001067717, -0.0010396060379057538, -0.0010706235080575682, -0.0010704488576227164, -0.0010349490619260794, -0.0009617969948137154, -0.0008508685989463798, -0.0007044682624354848, -0.0005273402858847709, -0.00032644791808218227, -0.00011052957492162269, 0.00011052957492162535, 0.0003264479180821849, 0.0005273402858847735, 0.0007044682624354872, 0.0008508685989463821, 0.0009617969948137175, 0.001034949061926081, 0.0010704488576227182, 0.00107062350805757, 0.0010396060379057551, 0.000982824500106773, 0.0009064422734077401, 0.0008168119984833217, 0.0007199955255671927, 0.0006213869286926815, 0.0005254580382221805, 0.000435628882066895, 0.00035425117413348636, 0.0002826829761606056, 0.0002214274015731739],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-1885320965307138871_q": {
            "type": "arbitrary",
            "samples": [0.0011180555187915678, 0.0015045081475491685, 0.0019931494632565378, 0.0025995569652476707, 0.0033378972155056613, 0.004219497446825634, 0.005251249417242201, 0.006433965280360931, 0.007760843544895696, 0.009216229744590979, 0.010774864198537374, 0.012401792672522064, 0.01405307218212388, 0.015677334902548905, 0.0172181839630987, 0.018617295840194316, 0.019818008261921365, 0.020769094336384877, 0.021428376161292048] + [0.02176582398456596] * 2 + [0.021428376161292048, 0.020769094336384877, 0.019818008261921365, 0.018617295840194316, 0.0172181839630987, 0.015677334902548905, 0.01405307218212388, 0.012401792672522064, 0.010774864198537374, 0.009216229744590979, 0.007760843544895696, 0.006433965280360931, 0.005251249417242201, 0.004219497446825634, 0.0033378972155056613, 0.0025995569652476707, 0.0019931494632565378, 0.0015045081475491685, 0.0011180555187915678],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-5492222400611121274": {
            "type": "arbitrary",
            "samples": [0.25] + [0.0] * 15,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-3597628208875341953_i": {
            "type": "constant",
            "sample": 0.0031,
        },
        "-3597628208875341953_q": {
            "type": "constant",
            "sample": 0.0,
        },
        "9181858318004187397_i": {
            "type": "arbitrary",
            "samples": [0.0001847802794911338, 0.0002358978110714119, 0.00029562118555058855, 0.00036353055679122937, 0.0004384926277133374, 0.0005185449024836567, 0.000600833381512208, 0.0006816263402774137, 0.0007564224456091563, 0.0008201631077734924, 0.000867547073578488, 0.0008934310281524939, 0.0008932852830643261, 0.0008636608457810521, 0.0008026157388505544, 0.0007100464369202522, 0.0005878759426368721, 0.0004400633558467776, 0.0002724194797661072, 9.223648744893448e-05, -9.22364874489315e-05, -0.00027241947976610427, -0.00044006335584677477, -0.0005878759426368692, -0.0007100464369202496, -0.000802615738850552, -0.0008636608457810499, -0.0008932852830643241, -0.0008934310281524921, -0.0008675470735784865, -0.0008201631077734911, -0.0007564224456091552, -0.0006816263402774129, -0.0006008333815122073, -0.0005185449024836561, -0.00043849262771333695, -0.00036353055679122905, -0.0002956211855505882, -0.0002358978110714117, -0.00018478027949113364],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "9181858318004187397_q": {
            "type": "arbitrary",
            "samples": [0.0012493039327485741, 0.0016811221929452756, 0.0022271250587747485, 0.0029047186704985986, 0.0037297326012476254, 0.004714823786424647, 0.00586769300677935, 0.007189247754539428, 0.008671888112107319, 0.010298121937161474, 0.01203972431763831, 0.013857637745628482, 0.01570276077461938, 0.017517695516533722, 0.019239424678246013, 0.020802778144181227, 0.022144442064578468, 0.023207176028356858, 0.023943850873928398] + [0.024320911659934084] * 2 + [0.023943850873928398, 0.023207176028356858, 0.022144442064578468, 0.020802778144181227, 0.019239424678246013, 0.017517695516533722, 0.01570276077461938, 0.013857637745628482, 0.01203972431763831, 0.010298121937161474, 0.008671888112107319, 0.007189247754539428, 0.00586769300677935, 0.004714823786424647, 0.0037297326012476254, 0.0029047186704985986, 0.0022271250587747485, 0.0016811221929452756, 0.0012493039327485741],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "191989489022899436_i": {
            "type": "constant",
            "sample": 0.0021,
        },
        "191989489022899436_q": {
            "type": "constant",
            "sample": 0.0,
        },
        "-3579424934842844975_i": {
            "type": "arbitrary",
            "samples": [0.00030171729028092756, 0.0003851842227735331, 0.0004827031504638384, 0.0005935885302881006, 0.0007159898654021671, 0.0008467027071136329, 0.000981066920557875, 0.0011129891833639896, 0.0012351195226318389, 0.0013391980526670656, 0.0014165686563095033, 0.001458833104968909, 0.0014585951260395835, 0.0014102230542588634, 0.0013105459442409456, 0.00115939475528007, 0.0009599094498731598, 0.0007185546187270059, 0.00044481839447977256, 0.00015060775497668046, -0.00015060775497667656, -0.00044481839447976866, -0.0007185546187270022, -0.0009599094498731563, -0.0011593947552800666, -0.0013105459442409426, -0.0014102230542588608, -0.001458595126039581, -0.001458833104968907, -0.0014165686563095015, -0.0013391980526670638, -0.0012351195226318376, -0.0011129891833639883, -0.0009810669205578741, -0.0008467027071136323, -0.0007159898654021665, -0.0005935885302881001, -0.0004827031504638381, -0.00038518422277353286, -0.00030171729028092735],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-3579424934842844975_q": {
            "type": "arbitrary",
            "samples": [0.0016409510779196645, 0.0022081410314293824, 0.002925311583561483, 0.00381532556527005, 0.004898974998745269, 0.006192884670996909, 0.007707169498132072, 0.009443021464190989, 0.011390458136038771, 0.01352650371967906, 0.015814085010857203, 0.018201900274243128, 0.02062545513863524, 0.023009357920839617, 0.025270835892482323, 0.02732428860950993, 0.029086553818695706, 0.030482446681665623, 0.031450063408251405] + [0.03194532984183734] * 2 + [0.031450063408251405, 0.030482446681665623, 0.029086553818695706, 0.02732428860950993, 0.025270835892482323, 0.023009357920839617, 0.02062545513863524, 0.018201900274243128, 0.015814085010857203, 0.01352650371967906, 0.011390458136038771, 0.009443021464190989, 0.007707169498132072, 0.006192884670996909, 0.004898974998745269, 0.00381532556527005, 0.002925311583561483, 0.0022081410314293824, 0.0016409510779196645],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-676466526619785515_i": {
            "type": "constant",
            "sample": 0.0017,
        },
        "-676466526619785515_q": {
            "type": "constant",
            "sample": 0.0,
        },
        "-5671910444091142186_i": {
            "type": "arbitrary",
            "samples": [0.0003245581697577897, 0.00041434379264958276, 0.0005192451877881844, 0.0006385249144990762, 0.000770192387926079, 0.0009108005732581571, 0.0010553365498202035, 0.0011972457129536531, 0.0013286216753579422, 0.0014405792538840596, 0.0015238070380387695, 0.001569271028816185, 0.0015690150341873355, 0.001516981055392401, 0.0014097580972250404, 0.0012471643221047002, 0.0010325773968537242, 0.0007729513005631867, 0.00047849244520435603, 0.00016200920159745652, -0.00016200920159745196, -0.0004784924452043516, -0.0007729513005631823, -0.0010325773968537198, -0.0012471643221046963, -0.001409758097225037, -0.0015169810553923976, -0.0015690150341873324, -0.0015692710288161824, -0.0015238070380387673, -0.0014405792538840579, -0.0013286216753579405, -0.0011972457129536518, -0.0010553365498202022, -0.0009108005732581562, -0.0007701923879260784, -0.0006385249144990755, -0.000519245187788184, -0.00041434379264958243, -0.0003245581697577895],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-5671910444091142186_q": {
            "type": "arbitrary",
            "samples": [0.0019126784932123073, 0.0025737902352024828, 0.0034097180757614314, 0.004447110734432495, 0.005710203213836212, 0.0072183732230314875, 0.008983410621423042, 0.011006704775378151, 0.013276620246507487, 0.015766376646515118, 0.018432761766696917, 0.021215978744641418, 0.02404085349478909, 0.026819510118246643, 0.02945546943320567, 0.031848956296730166, 0.033903037500186334, 0.03553007824122775, 0.036657923993314044] + [0.03723520229775363] * 2 + [0.036657923993314044, 0.03553007824122775, 0.033903037500186334, 0.031848956296730166, 0.02945546943320567, 0.026819510118246643, 0.02404085349478909, 0.021215978744641418, 0.018432761766696917, 0.015766376646515118, 0.013276620246507487, 0.011006704775378151, 0.008983410621423042, 0.0072183732230314875, 0.005710203213836212, 0.004447110734432495, 0.0034097180757614314, 0.0025737902352024828, 0.0019126784932123073],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "2330723448246942021_i": {
            "type": "constant",
            "sample": 0.0033,
        },
        "2330723448246942021_q": {
            "type": "constant",
            "sample": 0.0,
        },
        "-5270728341412406026": {
            "type": "arbitrary",
            "samples": [0.25] * 2 + [0.0] * 14,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-6418550158589274799": {
            "type": "arbitrary",
            "samples": [0.25] * 3 + [0.0] * 13,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-37518524385641680": {
            "type": "arbitrary",
            "samples": [0.25] * 4 + [0.0] * 12,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-8619147853570476064": {
            "type": "arbitrary",
            "samples": [0.25] * 5 + [0.0] * 11,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "2160241163617525183": {
            "type": "arbitrary",
            "samples": [0.25] * 6 + [0.0] * 10,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-192124505144769037": {
            "type": "arbitrary",
            "samples": [0.25] * 7 + [0.0] * 9,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "1233913405639371658": {
            "type": "arbitrary",
            "samples": [0.25] * 8 + [0.0] * 8,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "2005635182858397826": {
            "type": "arbitrary",
            "samples": [0.25] * 9 + [0.0] * 7,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "3431673093642538521": {
            "type": "arbitrary",
            "samples": [0.25] * 10 + [0.0] * 6,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "7238844999885162172": {
            "type": "arbitrary",
            "samples": [0.25] * 11 + [0.0] * 5,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "7460339059083877420": {
            "type": "arbitrary",
            "samples": [0.25] * 12 + [0.0] * 4,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "7183144655667875206": {
            "type": "arbitrary",
            "samples": [0.25] * 13 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-8788645326622507333": {
            "type": "arbitrary",
            "samples": [0.25] * 14 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "1464247641275318685": {
            "type": "arbitrary",
            "samples": [0.25] * 15 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-3555435509595742987": {
            "type": "constant",
            "sample": 0.25,
        },
        "966882260494260329": {
            "type": "arbitrary",
            "samples": [0.25] * 17 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-8403705110227432949": {
            "type": "arbitrary",
            "samples": [0.25] * 18 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-1136181762393860876": {
            "type": "arbitrary",
            "samples": [0.25] * 19 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "9116711205503965142": {
            "type": "constant",
            "sample": 0.25,
        },
        "1449356148982414773": {
            "type": "arbitrary",
            "samples": [0.25] * 21 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-7132273180202419611": {
            "type": "arbitrary",
            "samples": [0.25] * 22 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-751241545998786492": {
            "type": "arbitrary",
            "samples": [0.25] * 23 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-1899063363175655265": {
            "type": "constant",
            "sample": 0.25,
        },
        "-1677569303976940017": {
            "type": "arbitrary",
            "samples": [0.25] * 25 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-4002127386063776470": {
            "type": "arbitrary",
            "samples": [0.25] * 26 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "520190384026226846": {
            "type": "arbitrary",
            "samples": [0.25] * 27 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-7147164672495323523": {
            "type": "constant",
            "sample": 0.25,
        },
        "-1292629087581865633": {
            "type": "arbitrary",
            "samples": [0.25] * 29 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "4271661946051563531": {
            "type": "arbitrary",
            "samples": [0.25] * 30 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-5969552838430996306": {
            "type": "arbitrary",
            "samples": [0.25] * 31 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "1126624659620016478": {
            "type": "constant",
            "sample": 0.25,
        },
        "-2068560836227686534": {
            "type": "arbitrary",
            "samples": [0.25] * 33 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-2345755239643688748": {
            "type": "arbitrary",
            "samples": [0.25] * 34 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-2124261180444973500": {
            "type": "arbitrary",
            "samples": [0.25] * 35 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "8557594164649947687": {
            "type": "constant",
            "sample": 0.25,
        },
        "6205228495887653467": {
            "type": "arbitrary",
            "samples": [0.25] * 37 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "3880670413800817014": {
            "type": "arbitrary",
            "samples": [0.25] * 38 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-1739320964049899116": {
            "type": "arbitrary",
            "samples": [0.25] * 39 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "735633127369269961": {
            "type": "constant",
            "sample": 0.25,
        },
        "2161671038153410656": {
            "type": "arbitrary",
            "samples": [0.25] * 41 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-4589052013593251692": {
            "type": "arbitrary",
            "samples": [0.25] * 42 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-6198464231832708480": {
            "type": "arbitrary",
            "samples": [0.25] * 43 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-2391292325590084829": {
            "type": "constant",
            "sample": 0.25,
        },
        "8388096691597916418": {
            "type": "arbitrary",
            "samples": [0.25] * 45 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "7240274874421047645": {
            "type": "arbitrary",
            "samples": [0.25] * 46 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "5758536619419619984": {
            "type": "arbitrary",
            "samples": [0.25] * 47 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "5039677179439846380": {
            "type": "constant",
            "sample": 0.25,
        },
        "5261171238638561628": {
            "type": "arbitrary",
            "samples": [0.25] * 49 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "7625215090816122029": {
            "type": "arbitrary",
            "samples": [0.25] * 50 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-5257237949260000423": {
            "type": "arbitrary",
            "samples": [0.25] * 51 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-5035743890061285175": {
            "type": "constant",
            "sample": 0.25,
        },
        "-8402275235691547476": {
            "type": "arbitrary",
            "samples": [0.25] * 53 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-8180781176492832228": {
            "type": "arbitrary",
            "samples": [0.25] * 54 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "3016551382855339578": {
            "type": "arbitrary",
            "samples": [0.25] * 55 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "2395225614968646034": {
            "type": "constant",
            "sample": 0.25,
        },
        "2616719674167361282": {
            "type": "arbitrary",
            "samples": [0.25] * 57 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-749811671462901019": {
            "type": "arbitrary",
            "samples": [0.25] * 58 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "4814479362170528145": {
            "type": "arbitrary",
            "samples": [0.25] * 59 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-3379371743641197453": {
            "type": "constant",
            "sample": 0.25,
        },
        "3001659890562435666": {
            "type": "arbitrary",
            "samples": [0.25] * 61 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-3876737124422255809": {
            "type": "arbitrary",
            "samples": [0.25] * 62 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "6902651892765745438": {
            "type": "arbitrary",
            "samples": [0.25] * 63 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "2847053909803308309": {
            "type": "constant",
            "sample": 0.25,
        },
        "4273091820587449004": {
            "type": "arbitrary",
            "samples": [0.25] * 65 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "6748045912006618081": {
            "type": "arbitrary",
            "samples": [0.25] * 66 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "1128054534155901951": {
            "type": "arbitrary",
            "samples": [0.25] * 67 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-8168720658876312098": {
            "type": "constant",
            "sample": 0.25,
        },
        "-1072543160825299314": {
            "type": "arbitrary",
            "samples": [0.25] * 69 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-4267728656673002326": {
            "type": "arbitrary",
            "samples": [0.25] * 70 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-4046234597474287078": {
            "type": "arbitrary",
            "samples": [0.25] * 71 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-4323429000890289292": {
            "type": "constant",
            "sample": 0.25,
        },
        "-516257094647665641": {
            "type": "arbitrary",
            "samples": [0.25] * 73 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "4006060675442337675": {
            "type": "arbitrary",
            "samples": [0.25] * 74 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-3661294381079212694": {
            "type": "arbitrary",
            "samples": [0.25] * 75 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "3283707827749389633": {
            "type": "constant",
            "sample": 0.25,
        },
        "-7133674280342917920": {
            "type": "arbitrary",
            "samples": [0.25] * 77 + [0.0] * 3,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-6912180221144202672": {
            "type": "arbitrary",
            "samples": [0.25] * 78 + [0.0] * 2,
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-2389862451054199356": {
            "type": "arbitrary",
            "samples": [0.25] * 79 + [0.0],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "3274127046900791126_i": {
            "type": "arbitrary",
            "samples": [0.0011180555187915678, 0.0015045081475491685, 0.0019931494632565378, 0.0025995569652476707, 0.0033378972155056613, 0.004219497446825634, 0.005251249417242201, 0.006433965280360931, 0.007760843544895696, 0.009216229744590979, 0.010774864198537374, 0.012401792672522064, 0.01405307218212388, 0.015677334902548905, 0.0172181839630987, 0.018617295840194316, 0.019818008261921365, 0.020769094336384877, 0.021428376161292048] + [0.02176582398456596] * 2 + [0.021428376161292048, 0.020769094336384877, 0.019818008261921365, 0.018617295840194316, 0.0172181839630987, 0.015677334902548905, 0.01405307218212388, 0.012401792672522064, 0.010774864198537374, 0.009216229744590979, 0.007760843544895696, 0.006433965280360931, 0.005251249417242201, 0.004219497446825634, 0.0033378972155056613, 0.0025995569652476707, 0.0019931494632565378, 0.0015045081475491685, 0.0011180555187915678],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "3274127046900791126_q": {
            "type": "arbitrary",
            "samples": [0.0002214274015731738, 0.0002826829761606055, 0.00035425117413348626, 0.00043562888206689486, 0.0005254580382221803, 0.0006213869286926813, 0.0007199955255671924, 0.0008168119984833213, 0.0009064422734077396, 0.0009828245001067724, 0.0010396060379057545, 0.001070623508057569, 0.0010704488576227173, 0.0010349490619260802, 0.0009617969948137164, 0.000850868598946381, 0.000704468262435486, 0.0005273402858847722, 0.0003264479180821836, 0.00011052957492162402, -0.00011052957492162402, -0.0003264479180821836, -0.0005273402858847722, -0.000704468262435486, -0.000850868598946381, -0.0009617969948137164, -0.0010349490619260802, -0.0010704488576227173, -0.001070623508057569, -0.0010396060379057545, -0.0009828245001067724, -0.0009064422734077396, -0.0008168119984833213, -0.0007199955255671924, -0.0006213869286926813, -0.0005254580382221803, -0.00043562888206689486, -0.00035425117413348626, -0.0002826829761606055, -0.0002214274015731738],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-8503795126481802350_i": {
            "type": "arbitrary",
            "samples": [0.0012493039327485741, 0.0016811221929452756, 0.0022271250587747485, 0.0029047186704985986, 0.0037297326012476254, 0.004714823786424647, 0.00586769300677935, 0.007189247754539428, 0.008671888112107319, 0.010298121937161474, 0.01203972431763831, 0.013857637745628482, 0.01570276077461938, 0.017517695516533722, 0.019239424678246013, 0.020802778144181227, 0.022144442064578468, 0.023207176028356858, 0.023943850873928398] + [0.024320911659934084] * 2 + [0.023943850873928398, 0.023207176028356858, 0.022144442064578468, 0.020802778144181227, 0.019239424678246013, 0.017517695516533722, 0.01570276077461938, 0.013857637745628482, 0.01203972431763831, 0.010298121937161474, 0.008671888112107319, 0.007189247754539428, 0.00586769300677935, 0.004714823786424647, 0.0037297326012476254, 0.0029047186704985986, 0.0022271250587747485, 0.0016811221929452756, 0.0012493039327485741],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-8503795126481802350_q": {
            "type": "arbitrary",
            "samples": [-0.00018478027949113372, -0.0002358978110714118, -0.0002956211855505884, -0.0003635305567912292, -0.00043849262771333716, -0.0005185449024836564, -0.0006008333815122076, -0.0006816263402774133, -0.0007564224456091558, -0.0008201631077734918, -0.0008675470735784872, -0.000893431028152493, -0.0008932852830643251, -0.000863660845781051, -0.0008026157388505532, -0.0007100464369202509, -0.0005878759426368707, -0.0004400633558467762, -0.00027241947976610573, -9.223648744893299e-05, 9.223648744893299e-05, 0.00027241947976610573, 0.0004400633558467762, 0.0005878759426368707, 0.0007100464369202509, 0.0008026157388505532, 0.000863660845781051, 0.0008932852830643251, 0.000893431028152493, 0.0008675470735784872, 0.0008201631077734918, 0.0007564224456091558, 0.0006816263402774133, 0.0006008333815122076, 0.0005185449024836564, 0.00043849262771333716, 0.0003635305567912292, 0.0002956211855505884, 0.0002358978110714118, 0.00018478027949113372],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-2818334305619283106_i": {
            "type": "arbitrary",
            "samples": [0.0016409510779196645, 0.0022081410314293824, 0.002925311583561483, 0.00381532556527005, 0.004898974998745269, 0.006192884670996909, 0.007707169498132072, 0.009443021464190989, 0.011390458136038771, 0.01352650371967906, 0.015814085010857203, 0.018201900274243128, 0.02062545513863524, 0.023009357920839617, 0.025270835892482323, 0.02732428860950993, 0.029086553818695706, 0.030482446681665623, 0.031450063408251405] + [0.03194532984183734] * 2 + [0.031450063408251405, 0.030482446681665623, 0.029086553818695706, 0.02732428860950993, 0.025270835892482323, 0.023009357920839617, 0.02062545513863524, 0.018201900274243128, 0.015814085010857203, 0.01352650371967906, 0.011390458136038771, 0.009443021464190989, 0.007707169498132072, 0.006192884670996909, 0.004898974998745269, 0.00381532556527005, 0.002925311583561483, 0.0022081410314293824, 0.0016409510779196645],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-2818334305619283106_q": {
            "type": "arbitrary",
            "samples": [-0.00030171729028092745, -0.00038518422277353297, -0.00048270315046383824, -0.0005935885302881004, -0.0007159898654021668, -0.0008467027071136326, -0.0009810669205578746, -0.001112989183363989, -0.0012351195226318382, -0.0013391980526670647, -0.0014165686563095024, -0.001458833104968908, -0.0014585951260395822, -0.001410223054258862, -0.001310545944240944, -0.0011593947552800683, -0.000959909449873158, -0.000718554618727004, -0.0004448183944797706, -0.0001506077549766785, 0.0001506077549766785, 0.0004448183944797706, 0.000718554618727004, 0.000959909449873158, 0.0011593947552800683, 0.001310545944240944, 0.001410223054258862, 0.0014585951260395822, 0.001458833104968908, 0.0014165686563095024, 0.0013391980526670647, 0.0012351195226318382, 0.001112989183363989, 0.0009810669205578746, 0.0008467027071136326, 0.0007159898654021668, 0.0005935885302881004, 0.00048270315046383824, 0.00038518422277353297, 0.00030171729028092745],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-4910819814867580317_i": {
            "type": "arbitrary",
            "samples": [0.0019126784932123073, 0.0025737902352024828, 0.0034097180757614314, 0.004447110734432495, 0.005710203213836212, 0.0072183732230314875, 0.008983410621423042, 0.011006704775378151, 0.013276620246507487, 0.015766376646515118, 0.018432761766696917, 0.021215978744641418, 0.02404085349478909, 0.026819510118246643, 0.02945546943320567, 0.031848956296730166, 0.033903037500186334, 0.03553007824122775, 0.036657923993314044] + [0.03723520229775363] * 2 + [0.036657923993314044, 0.03553007824122775, 0.033903037500186334, 0.031848956296730166, 0.02945546943320567, 0.026819510118246643, 0.02404085349478909, 0.021215978744641418, 0.018432761766696917, 0.015766376646515118, 0.013276620246507487, 0.011006704775378151, 0.008983410621423042, 0.0072183732230314875, 0.005710203213836212, 0.004447110734432495, 0.0034097180757614314, 0.0025737902352024828, 0.0019126784932123073],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
        "-4910819814867580317_q": {
            "type": "arbitrary",
            "samples": [-0.0003245581697577896, -0.0004143437926495826, -0.0005192451877881842, -0.0006385249144990759, -0.0007701923879260787, -0.0009108005732581567, -0.0010553365498202029, -0.0011972457129536525, -0.0013286216753579413, -0.0014405792538840587, -0.0015238070380387684, -0.0015692710288161837, -0.001569015034187334, -0.0015169810553923994, -0.0014097580972250387, -0.0012471643221046982, -0.001032577396853722, -0.0007729513005631845, -0.0004784924452043538, -0.00016200920159745424, 0.00016200920159745424, 0.0004784924452043538, 0.0007729513005631845, 0.001032577396853722, 0.0012471643221046982, 0.0014097580972250387, 0.0015169810553923994, 0.001569015034187334, 0.0015692710288161837, 0.0015238070380387684, 0.0014405792538840587, 0.0013286216753579413, 0.0011972457129536525, 0.0010553365498202029, 0.0009108005732581567, 0.0007701923879260787, 0.0006385249144990759, 0.0005192451877881842, 0.0004143437926495826, 0.0003245581697577896],
            "is_overridable": False,
            "max_allowed_error": 0.0001,
        },
    },
    "digital_waveforms": {
        "ON": {
            "samples": [(1, 0)],
        },
    },
    "integration_weights": {
        "cosine_weights_B1/acquisition": {
            "cosine": [(1.0, 2000)],
            "sine": [(-0.0, 2000)],
        },
        "sine_weights_B1/acquisition": {
            "cosine": [(0.0, 2000)],
            "sine": [(1.0, 2000)],
        },
        "minus_sine_weights_B1/acquisition": {
            "cosine": [(-0.0, 2000)],
            "sine": [(-1.0, 2000)],
        },
        "cosine_weights_B2/acquisition": {
            "cosine": [(1.0, 2000)],
            "sine": [(-0.0, 2000)],
        },
        "sine_weights_B2/acquisition": {
            "cosine": [(0.0, 2000)],
            "sine": [(1.0, 2000)],
        },
        "minus_sine_weights_B2/acquisition": {
            "cosine": [(-0.0, 2000)],
            "sine": [(-1.0, 2000)],
        },
        "cosine_weights_B3/acquisition": {
            "cosine": [(1.0, 2000)],
            "sine": [(-0.0, 2000)],
        },
        "sine_weights_B3/acquisition": {
            "cosine": [(0.0, 2000)],
            "sine": [(1.0, 2000)],
        },
        "minus_sine_weights_B3/acquisition": {
            "cosine": [(-0.0, 2000)],
            "sine": [(-1.0, 2000)],
        },
        "cosine_weights_B4/acquisition": {
            "cosine": [(1.0, 2000)],
            "sine": [(-0.0, 2000)],
        },
        "sine_weights_B4/acquisition": {
            "cosine": [(0.0, 2000)],
            "sine": [(1.0, 2000)],
        },
        "minus_sine_weights_B4/acquisition": {
            "cosine": [(-0.0, 2000)],
            "sine": [(-1.0, 2000)],
        },
    },
    "mixers": {
        "B1/drive_mixer_288": [{'intermediate_frequency': 100388701.0, 'lo_frequency': 4900000000.0, 'correction': (1, 0, 0, 1)}],
        "B4/acquisition_mixer_9da": [{'intermediate_frequency': 330300527.0, 'lo_frequency': 7370000000.0, 'correction': (1, 0, 0, 1)}],
        "B3/acquisition_mixer_9a4": [{'intermediate_frequency': 110622376.0, 'lo_frequency': 7370000000.0, 'correction': (1, 0, 0, 1)}],
        "B3/drive_mixer_223": [{'intermediate_frequency': -115376210.0, 'lo_frequency': 5800000000.0, 'correction': (1, 0, 0, 1)}],
        "B1/acquisition_mixer_db6": [{'intermediate_frequency': -237451236.0, 'lo_frequency': 7370000000.0, 'correction': (1, 0, 0, 1)}],
        "B4/drive_mixer_7be": [{'intermediate_frequency': 109615374.0, 'lo_frequency': 6700000000.0, 'correction': (1, 0, 0, 1)}],
        "B2/drive_mixer_3c3": [{'intermediate_frequency': 63761228.0, 'lo_frequency': 5900000000.0, 'correction': (1, 0, 0, 1)}],
        "B2/acquisition_mixer_5fd": [{'intermediate_frequency': 10040944.0, 'lo_frequency': 7370000000.0, 'correction': (1, 0, 0, 1)}],
    },
}

